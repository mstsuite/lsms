...
#if defined(CHECK) || defined(MAKE_BGIJ_CPU)
...
/*********************************************************************************************************/
...
#ifdef MAKE_BGIJ_CPU
  // JLARKIN - It appears that makeBGijs_cuda_batched may have a bug, this can
  // instead be constructed on the host. WAR
  // loop over LIZ to make each gij
  for(int ir1=0; ir1<atom.numLIZ; ir1++) //SERIAL
  {
    ...
    // copy up all gijs
    cudaMemcpy(dev_bgij+4*kkrsz*kkrsz*atom.numLIZ*ir1, &bgij[0],sizeof(Complex)*4*kkrsz*kkrsz*atom.numLIZ,cudaMemcpyHostToDevice);
  }
  cudaCheckError();
#else
