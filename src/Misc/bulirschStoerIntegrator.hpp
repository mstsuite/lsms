...
class ExtrapolatorT0 {
 public:
  inline int idx(int d, int i, int j) {
    return d + dim * (i * (i + 1) / 2 + j);
  }
  std::vector<Rf> T;
  std::vector<Rh> h;
  int kNum, dim;
  ExtrapolatorT0(int d, int kMax) {
    T.resize(d * (kMax + 1) * (kMax + 2) / 2);
    h.resize(kMax + 1);
    kNum = 0;
    dim = d;
  }
  ...
  void add(Rh hNew, Rf *fNew) {
    ...
    for (int d = 0; d < dim; d++) {
      ...
      for (int k = 1; k < kNum; k++) {
        ...
        // printf("den=%g\n",den);
        T[idx(d, i, k)] = T[idx(d, i, k - 1)] +
                          (T[idx(d, i, k - 1)] - T[idx(d, i - 1, k - 1)]) * den;
      }
    }
  }
  Rh error() {
    ...
    Rh err = Rh(0);
    for (int d = 0; d < dim; d++)
      err += std::abs(T[idx(d, kNum - 1, kNum - 1)] -
                      T[idx(d, kNum - 2, kNum - 2)]);
    return err;
  }
  Rh relativeError() {
    Rh mag = Rh(0);
  ...
  void estimate(Rf *f) {
    for (int d = 0; d < dim; d++) f[d] = T[idx(d, kNum - 1, kNum - 1)];
  }
};
...
template <typename Rx, typename Ry>
int bulirschStoerIntegrator(Rx x0, Rx x1, Ry *y0, Ry *y1, int n,
...
template <typename Rx, typename Ry>
int generalizedBulirschStoerIntegrator(
    Rx x0, Rx x1, Ry *y0, Ry *y1, int n,
    std::function<void(Rx x, Ry *y, Ry *dy)> rhs,
