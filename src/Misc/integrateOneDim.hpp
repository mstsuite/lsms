...
Real integrateStep(Real x0, Real x1, Fn &fn) {
  ...
  //  return
  //  nintieth*h*(7.0*fn(x0)+32.0*fn(x0+0.25*h)+12.0*fn(0.5*(x0+x1))+32.0*fn(x0+0.75*h)+7.0*fn(x1));
}
...
void integrateOneDim(std::vector<Real> &grid, Func fn,
...
template <size_t stepSubdivision = DEFAULT_STEP_SUBDIVISION>
inline void integrateOneDim(std::vector<Real> &grid,
                            std::vector<Real> &integrand,
                            std::vector<Real> &integral) {
  integrateOneDim<stepSubdivision>(&grid[0], &integrand[0], &integral[0],
                                   integrand.size());
}

template <size_t stepSubdivision = DEFAULT_STEP_SUBDIVISION>
void integrateOneDimSpherical(Real *grid, Real *integrand, Real *integral,
                              size_t n) {
  RationalFit<Real> fit;
  const Real invStepSubdivision = 1.0 / Real(stepSubdivision);
  integral[0] = 0.0;
...
template <size_t stepSubdivision = DEFAULT_STEP_SUBDIVISION>
inline void integrateOneDimSpherical(std::vector<Real> &grid,
                                     std::vector<Real> &integrand,
                                     std::vector<Real> &integral) {
  integrateOneDimSpherical<stepSubdivision>(&grid[0], &integrand[0],
                                            &integral[0], integrand.size());
}

template <size_t stepSubdivision = DEFAULT_STEP_SUBDIVISION>
Real integrateOneDim(std::vector<Real> &grid, std::vector<Real> &integrand,
                     std::vector<Real> &integral, Real r) {
  integrateOneDim<stepSubdivision>(grid, integrand, integral);
  return interpolate(grid, integral, r);
}

template <size_t stepSubdivision = DEFAULT_STEP_SUBDIVISION>
Real integrateOneDimSpherical(std::vector<Real> &grid,
                              std::vector<Real> &integrand,
                              std::vector<Real> &integral, Real r) {
...
template <size_t stepSubdivision = DEFAULT_STEP_SUBDIVISION>
void integrateOneDimRPower(Real *grid, Real *integrand, Real *integral,
                           size_t n, int p) {
  RationalFit<Real> fit;
  const Real invStepSubdivision = 1.0 / Real(stepSubdivision);
  integral[0] = 0.0;
  ...
  if (stepSubdivision == 0)  // use fit.integral for integration steps
    for (size_t i = 0; i < n - 1; i++) {
      fit.set(
          grid, integrand, [p](Real r, Real f) { return std::pow(r, p) * f; },
          i, n);
      integral[i + 1] = integral[i] + fit.integral(grid[i], grid[i + 1]);
    }
  else if (stepSubdivision == 1)
...
template <size_t stepSubdivision = DEFAULT_STEP_SUBDIVISION>
inline void integrateOneDimRPower(std::vector<Real> &grid,
                                  std::vector<Real> &integrand,
                                  std::vector<Real> &integral, int p) {
  integrateOneDimRPower<stepSubdivision>(&grid[0], &integrand[0], &integral[0],
                                         integrand.size(), p);
}

template <size_t stepSubdivision = DEFAULT_STEP_SUBDIVISION>
Real integrateOneDimRPower(std::vector<Real> &grid,
                           std::vector<Real> &integrand,
                           std::vector<Real> &integral, Real r, int p) {
