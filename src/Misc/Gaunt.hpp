...
Int binomialCoefficient(Int n, Int k) {
  ...
  Int b = Int(1);
  for (Int i = Int(1); i <= k; i++) b = (b * ((n + 1) - i)) / i;
...
Int w3j_intterm(Int j1, Int j2, Int j3, Int m1, Int m2, Int m3) {
  ...
  for (Int k = I1; k <= I2; k++) {
    sum += sgn * binomialCoefficient(j1 + j2 - j3, k) *
           binomialCoefficient(j1 - j2 + j3, j1 - m1 - k) *
...
Int w3j_sqrt_numerator(Int j1, Int j2, Int j3, Int m1, Int m2, Int m3) {
  return binomialCoefficient(Int(2) * j1, j1 - j2 + j3) *
         binomialCoefficient(Int(2) * j2, j1 + j2 - j3) *
         binomialCoefficient(Int(2) * j3, -j1 + j2 + j3);
}

template <typename Int>
Int w3j_sqrt_denominator(Int j1, Int j2, Int j3, Int m1, Int m2, Int m3) {
  return binomialCoefficient(Int(2) * j1, j1 + m1) *
         binomialCoefficient(Int(2) * j2, j2 + m2) *
...
R gaunt(Int lp, Int l1, Int l2, Int mp, Int m1, Int m2) {
  ...
  if (NewGaunt::iabs(mp) > lp || NewGaunt::iabs(m1) > l1 ||
      NewGaunt::iabs(m2) > l2)
    return R(0);
  R g = sqrt(R((2 * lp + 1) * (2 * l1 + 1) * (2 * l2 + 1)) / (4.0 * M_PI)) *
        NewGaunt::w3j<R, Int>(lp, l1, l2, 0, 0, 0) *
        NewGaunt::w3j<R, Int>(lp, l1, l2, -mp, m1, m2);
