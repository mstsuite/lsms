...
namespace lsms {
  ...
  T simpson_nonuniform(std::vector<T> x, std::vector<T> f, U length) {
    ...
    for (U i = 1; i < N; i += 2) {
      ...
      result += f[i] * (lsms::pow(h[i], 3) + lsms::pow(h[i - 1], 3)
                        + 3.0 * h[i] * h[i - 1] * hph)
                / (6 * h[i] * h[i - 1]);

      result += f[i - 1] * (2.0 * lsms::pow(h[i - 1], 3) - lsms::pow(h[i], 3)
                            + 3.0 * h[i] * lsms::pow(h[i - 1], 2))
                / (6 * h[i - 1] * hph);

      result += f[i + 1] * (2.0 * lsms::pow(h[i], 3) - lsms::pow(h[i - 1], 3)
    ...
    if ((N + 1) % 2 == 0) {
      ...
      if (N > 1) {
        ...
        result += f[N] * (2 * lsms::pow(h[N - 1], 2)
                          + 3.0 * h[N - 2] * h[N - 1])
                  / (6 * (h[N - 2] + h[N - 1]));

        result += f[N - 1] * (lsms::pow(h[N - 1], 2)
                              + 3.0 * h[N - 1] * h[N - 2])
                  / (6 * h[N - 2]);
  ...
  T radialIntegral(const std::vector<T> &integrand,
    ...
    for (int i = 1; i < extended_length; ++i) {
      integral[i] = simpson_nonuniform(extended_r_mesh, extended_integrand, i + 1);
    }
