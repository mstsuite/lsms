...
class RationalFit {
 public:
  ...
  void setWithStride(T *r, T *f, int i0, int n, int stride) {
    ...
    if (i1 != i2) {
      c[1] = (f[i2 * stride] - f[ip1 * stride]) / (r[i2] - r[ip1]) -
             (f[i1 * stride] - f[i0 * stride]) / (r[i1] - r[i0]);
      if (c[1] * (f[i2 * stride] - f[i1 * stride]) * (r[i2] - r[i1]) > 0.0)
        c[1] = -c[1];
      T h1 = r[i1] - r[i0];
  ...
  void set(T *r, T *f, Func fn, int i0, int n) {
    std::vector<T> ff;
    ff.resize(n);
    for (int i = 0; i < n; i++) ff[i] = fn(r[i], f[i]);
  ...
  void set(std::vector<T> &r, Func fn, int i0) {
    std::vector<T> f;
    f.resize(r.size());
    for (int i = 0; i < r.size(); i++) f[i] = fn(r[i], i);
  ...
  T operator()(T r) {
    T x = r - r0;
    return f0 - c[0] + c[1] * x +
           ((c[2] - c[1]) * x + c[0]) / (1.0 + c[3] * x * (delta - x));
  }
  ...
  T derivative(T r) {
    ...
    T fac = c[2] * delta - c[0];
    return c[1] + ((c[2] - c[1]) * den -
                   ((c[2] - c[1]) * x + c[0]) * c[3] * (delta - 2.0 * x)) /
                      (den * den);
  }
  ...
  T integral(T r1, T r2) {
    ...
    if (c[3] == 0.0) {
      ...
      {
        i2 = ((m / (2.0 * cc)) *
                  std::log(std::fabs(1.0 + 2.0 * b * x2 + cc * x2 * x2)) +
              ((n * cc - m * b) / (cc * std::sqrt(cc - b * b))) *
                  std::atan((cc * x2 + b) / std::sqrt(cc - b * b))) -
             ((m / (2.0 * cc)) *
                  std::log(std::fabs(1.0 + 2.0 * b * x1 + cc * x1 * x1)) +
              ((n * cc - m * b) / (cc * std::sqrt(cc - b * b))) *
                  std::atan((cc * x1 + b) / std::sqrt(cc - b * b)));
      } else {  // ac<b^2
        T b2ac = std::sqrt(b * b - cc);
        i2 = ((m / (2.0 * cc)) *
                  std::log(std::fabs(1.0 + 2.0 * b * x2 + cc * x2 * x2)) +
              ((n * cc - m * b) / (2.0 * cc * b2ac)) *
                  std::log(
                      std::fabs((cc * x2 + b - b2ac) / (cc * x2 + b + b2ac)))) -
             ((m / (2.0 * cc)) *
                  std::log(std::fabs(1.0 + 2.0 * b * x1 + cc * x1 * x1)) +
              ((n * cc - m * b) / (2.0 * cc * b2ac)) *
                  std::log(
                      std::fabs((cc * x1 + b - b2ac) / (cc * x1 + b + b2ac))));
      }
    }
...
// Interpolate from a table of points on a grid xOrigin to a different grid
// xTarget
template <typename T>
void interpolateTable(std::vector<T> &xOrigin, std::vector<T> &fOrigin,
                      std::vector<T> &xTarget, std::vector<T> &fTarget) {
...
template <typename T>
void calculateDerivative(T *r, T *f, T *df, size_t n, size_t strideF,
                         size_t strideDf) {
  RationalFit<T> fit;
  for (size_t i = 0; i < n; i++) {
