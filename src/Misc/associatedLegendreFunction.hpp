...
#ifndef LSMS_LEGENDREFUNCTIONS_H
#define LSMS_LEGENDREFUNCTIONS_H
...
//
// Plm is a 1-d array that will contain all the values of P_{lm}(x) from P_{00}
// to P_{l_{max} l_{max}} the index into this array is Plm[l*(l+1)/2 + m]
//

static inline int plmIdx(int l, int m) { return l * (l + 1) / 2 + m; }

template<typename R>
// R has to be real data type (default double)
void associatedLegendreFunctionNormalized(R x, int lmax, R *Plm) {
  ...
  for (int m = 1; m <= lmax; m++) {
    ...
    Plm[plmIdx(m, m - 1)] =
        std::sqrt(R(2 * m + 1)) * x * Plm[plmIdx(m - 1, m - 1)];
  }
...
void associatedLegendreFunction(R x, int lmax, R *Plm) {
  ...
  for (int m = 0; m < lmax; m++) {
    for (int l = m + 2; l <= lmax; l++) {
      ...
      Plm[plmIdx(l, m)] = (R(2 * l - 1) * x * Plm[plmIdx(l - 1, m)] -
                           R(l + m - 1) * Plm[plmIdx(l - 2, m)]) /
                          R(l - m);
    }
  }
}
