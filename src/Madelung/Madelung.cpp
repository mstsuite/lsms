...
void calculateMadelungMatrices(LSMSSystemParameters &lsms,
  ...
  //#pragma omp parallel for default(none) shared(lsms, crystal, local,
  //atom_position_1, atom_position_2, atom_position_3) \
//   firstprivate(num_atoms)
  for (int i = 0; i < local.num_local; i++) {
    int mynod = local.global_id[i];
    cal_madelung_matrix_(&mynod, &num_atoms, &crystal.bravais(0, 0),
                         &atom_position_1[0], &atom_position_2[0],
                         &atom_position_3[0], &local.atom[i].madelungMatrix[0],
