...
class REWLCommunication {
 public:
  ...
  template <typename T>
  void swapScalar(T &data, int partner);
  template <typename T>
  void swapVector(T data[], int nElements, int partner);

  template <typename T>
  void sendScalar(T &data, int partner);
  template <typename T>
  void recvScalar(T &data, int partner);
};
...
void REWLCommunication::swapScalar(T &data, int partner) {
  MPI_Status status;
  MPI_Sendrecv_replace(&data, 1, TypeTraits<T>::mpiType(), partner, 1, partner,
                       1, comm, &status);
}
...
void REWLCommunication::swapVector(T data[], int nElements, int partner) {
  MPI_Status status;
  MPI_Sendrecv_replace(&data[0], nElements, TypeTraits<T>::mpiType(), partner,
                       1, partner, 1, comm, &status);
}
