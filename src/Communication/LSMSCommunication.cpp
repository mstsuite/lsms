...
void finalizeCommunication() { MPI_Finalize(); }

void exitLSMS(LSMSCommunication &comm, int errorCode) {
  MPI_Abort(comm.comm, errorCode);
}

void synchronizeLSMS(LSMSCommunication &comm) { MPI_Barrier(comm.comm); }
...
void communicateParameters(LSMSCommunication &comm, LSMSSystemParameters &lsms,
  ...
  if (comm.rank == 0) {
    ...
    MPI_Pack(&lsms.lsmsAlgorithms, LSMSAlgorithms::numAlgorithms,
             MPI_SIGNED_CHAR, buf, s, &pos, comm.comm);
    ...
    MPI_Pack(&lsms.xcFunctional[0], numFunctionalIndices, MPI_INT, buf, s, &pos,
             comm.comm);
    ...
    MPI_Pack(&lsms.global.debug_core_states, 1, MPI_CXX_BOOL, buf, s, &pos,comm.comm);
    MPI_Pack(&lsms.global.debug_radial_charge, 1, MPI_CXX_BOOL, buf, s, &pos,
             comm.comm);
    MPI_Pack(&lsms.global.debug_charge, 1, MPI_CXX_BOOL, buf, s, &pos,
             comm.comm);
    MPI_Pack(&lsms.global.debug_potential, 1, MPI_CXX_BOOL, buf, s, &pos,
             comm.comm);
    MPI_Pack(&lsms.global.debug_energy, 1, MPI_CXX_BOOL, buf, s, &pos,
             comm.comm);
    MPI_Pack(&lsms.global.debug_convergence, 1, MPI_CXX_BOOL, buf, s, &pos,
             comm.comm);
    ...
    // MixingParameters
    MPI_Pack(&mix.n_init_spin_iterations, 1, MPI_UNSIGNED, buf, s, &pos, comm.comm);
    MPI_Pack(&mix.n_init_iterations, 1, MPI_UNSIGNED, buf, s, &pos, comm.comm);
    MPI_Pack(&mix.chd_mixer_type, 1, MPI_UNSIGNED, buf, s, &pos, comm.comm);
    MPI_Pack(&mix.spd_mixer_type, 1, MPI_UNSIGNED, buf, s, &pos, comm.comm);
    MPI_Pack(&mix.pot_mixer_type, 1, MPI_UNSIGNED, buf, s, &pos, comm.comm);
    ...
    MPI_Pack(&mix.initSpdMixingParameter.max_broyden, 1, MPI_UNSIGNED, buf, s, &pos,
             comm.comm);
    MPI_Pack(&mix.initSpdMixingParameter.iter_reset, 1, MPI_UNSIGNED, buf, s, &pos,
             comm.comm);
  ...
  if (comm.rank != 0) {
    ...
    MPI_Unpack(buf, s, &pos, &lsms.lsmsAlgorithms,
               LSMSAlgorithms::numAlgorithms, MPI_SIGNED_CHAR, comm.comm);
    ...
    MPI_Unpack(buf, s, &pos, &lsms.xcFunctional[0], numFunctionalIndices,
               MPI_INT, comm.comm);
    ...
    MPI_Unpack(buf, s, &pos, &lsms.global.default_iprint, 1, MPI_INT,
               comm.comm);
    ...
    MPI_Unpack(buf, s, &pos, &lsms.global.debug_chem_pot, 1, MPI_CXX_BOOL,
               comm.comm);
    MPI_Unpack(buf, s, &pos, &lsms.global.debug_madelung, 1, MPI_CXX_BOOL,
               comm.comm);
    MPI_Unpack(buf, s, &pos, &lsms.global.debug_core_states, 1, MPI_CXX_BOOL,
               comm.comm);
    MPI_Unpack(buf, s, &pos, &lsms.global.debug_radial_charge, 1, MPI_CXX_BOOL,
               comm.comm);
    MPI_Unpack(buf, s, &pos, &lsms.global.debug_charge, 1, MPI_CXX_BOOL,
               comm.comm);
    MPI_Unpack(buf, s, &pos, &lsms.global.debug_potential, 1, MPI_CXX_BOOL,
               comm.comm);
    MPI_Unpack(buf, s, &pos, &lsms.global.debug_energy, 1, MPI_CXX_BOOL,
               comm.comm);
    MPI_Unpack(buf, s, &pos, &lsms.global.debug_convergence, 1, MPI_CXX_BOOL,
               comm.comm);
    ...
    // MixingParameters
    MPI_Unpack(buf, s, &pos, &mix.n_init_spin_iterations, 1, MPI_UNSIGNED, comm.comm);
    MPI_Unpack(buf, s, &pos, &mix.n_init_iterations, 1, MPI_UNSIGNED, comm.comm);
    MPI_Unpack(buf, s, &pos, &mix.chd_mixer_type, 1, MPI_UNSIGNED, comm.comm);
    MPI_Unpack(buf, s, &pos, &mix.spd_mixer_type, 1, MPI_UNSIGNED, comm.comm);
    MPI_Unpack(buf, s, &pos, &mix.pot_mixer_type, 1, MPI_UNSIGNED, comm.comm);
    ...
    MPI_Unpack(buf, s, &pos, &mix.initSpdMixingParameter.max_broyden, 1,
               MPI_UNSIGNED, comm.comm);
    MPI_Unpack(buf, s, &pos, &mix.initSpdMixingParameter.iter_reset, 1,
               MPI_UNSIGNED, comm.comm);


  }
  lsms.rank = comm.rank;
  MPI_Bcast(&crystal.position(0, 0), 3 * crystal.num_atoms, MPI_DOUBLE, 0,
            comm.comm);
  MPI_Bcast(&crystal.evecs(0, 0), 3 * crystal.num_atoms, MPI_DOUBLE, 0,
            comm.comm);
  MPI_Bcast(&crystal.type[0], crystal.num_atoms, MPI_INT, 0, comm.comm);

  // This is dangerous and assumes homogeneous nodes:
  MPI_Bcast(&crystal.types[0], crystal.num_types * sizeof(AtomType), MPI_BYTE,
            0, comm.comm);
  ...
  if (comm.rank == 0)
    for (int i = 0; i < alloyDesc.size(); i++) ncomps[i] = alloyDesc[i].size();

  MPI_Bcast(ncomps, nalloy_classes, MPI_INT, 0, comm.comm);

  if (comm.rank != 0)
    for (int i = 0; i < alloyDesc.size(); i++) alloyDesc[i].resize(ncomps[i]);

  for (int i = 0; i < alloyDesc.size(); i++)
    MPI_Bcast(alloyDesc[i].data(), ncomps[i] * sizeof(AtomType), MPI_BYTE, 0,
              comm.comm);  // <-- see danger above
  free(ncomps);

  // get maximum lmax
  crystal.maxlmax = 0;
  for (int i = 0; i < crystal.num_types; i++)
    if (crystal.types[i].lmax > crystal.maxlmax)
      crystal.maxlmax = crystal.types[i].lmax;
  lsms.maxlmax = crystal.maxlmax;

  for (int i = 0; i < alloyDesc.size(); i++) {
    for (int j = 0; j < alloyDesc[i].size(); j++) {
      if (alloyDesc[i][j].lmax > lsms.maxlmax)
        lsms.maxlmax = alloyDesc[i][j].lmax;
    }
  }
                           ...
                           AlloyMixingDesc &alloyDesc)
{
  ...
  if(comm.rank==0)
  {
    ...
    MPI_Pack(lsms.title,80,MPI_CHAR,buf,s,&pos,comm.comm);
    MPI_Pack(&lsms.lsmsMode,1,MPI_INT,buf,s,&pos,comm.comm);
    MPI_Pack(&lsms.lsmsAlgorithms,LSMSAlgorithms::numAlgorithms,MPI_SIGNED_CHAR,buf,s,&pos,comm.comm);
    MPI_Pack(lsms.potential_file_in,128,MPI_CHAR,buf,s,&pos,comm.comm);
    MPI_Pack(lsms.potential_file_out,128,MPI_CHAR,buf,s,&pos,comm.comm);
    MPI_Pack(&lsms.pot_in_type,1,MPI_INT,buf,s,&pos,comm.comm);
    MPI_Pack(&lsms.pot_out_type,1,MPI_INT,buf,s,&pos,comm.comm);
...
// MixingParameters
  ...
  if(comm.rank!=0)
  {
    ...
    MPI_Unpack(buf,s,&pos,lsms.title,80,MPI_CHAR,comm.comm);
    MPI_Unpack(buf,s,&pos,&lsms.lsmsMode,1,MPI_INT,comm.comm);
    MPI_Unpack(buf,s,&pos,&lsms.lsmsAlgorithms,LSMSAlgorithms::numAlgorithms,MPI_SIGNED_CHAR,comm.comm);
    MPI_Unpack(buf,s,&pos,lsms.potential_file_in,128,MPI_CHAR,comm.comm);
    MPI_Unpack(buf,s,&pos,lsms.potential_file_out,128,MPI_CHAR,comm.comm);
    MPI_Unpack(buf,s,&pos,&lsms.pot_in_type,1,MPI_INT,comm.comm);
    MPI_Unpack(buf,s,&pos,&lsms.pot_out_type,1,MPI_INT,comm.comm);
...
void communicateSingleAtomData(LSMSCommunication &comm, int from, int to,
                               int &local_id, AtomData &atom, int tag) {
  const int maxPts = 3051;
  const int maxCore = 30;
  int s = sizeof(AtomData) + sizeof(Real) * (2 * 3 * maxPts + 2 * maxCore) +
          sizeof(int) * 3 * 2 * maxCore + sizeof(int);
  char buf[s];
  int t;
...
void printCommunicationInfo(FILE *f, LSMSCommunication &comm) {
  ...
  for (int i = 0; i < comm.numTmatTo; i++) {
    ...
    for (int j = 0; j < comm.tmatTo[i].numTmats; j++)
      fprintf(f, " %d[%d]", comm.tmatTo[i].globalIdx[j],
              comm.tmatTo[i].tmatStoreIdx[j]);
    fprintf(f, "\n");
  }
  ...
  for (int i = 0; i < comm.numTmatFrom; i++) {
    ...
    for (int j = 0; j < comm.tmatFrom[i].numTmats; j++)
      fprintf(f, " %d[%d]", comm.tmatFrom[i].globalIdx[j],
              comm.tmatFrom[i].tmatStoreIdx[j]);
    fprintf(f, "\n");
  }
}
