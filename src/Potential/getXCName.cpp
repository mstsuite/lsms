...
bool getXCName(LSMSSystemParameters &lsms, std::string &name) {
  if (lsms.xcFunctional[0] == 0)  // built in functionals
  {
    switch (lsms.xcFunctional[1]) {
      case 1:
        name = "von Barth-Hedin (LSMS_1)";
        return true;
      case 2:
        name = "Vosko-Wilk-Nusair (LSMS_1)";
        return true;
    }
    name = "Illegal Exchange-Correlation Functional (built in)!";
    return false;
...
#else
    name =
        "Illegal Exchange-Correlation Functional (LSMS not linked to libXC)!";
    return false;
#endif
    ...
    switch (lsms.xcFunctional[1]) {
      ...
      case 2:
        name = "Vosko-Wilk-Nusair (LSMS_3)";
        return true;
    }
    name = "Illegal Exchange-Correlation Functional (LSMS_3)!";
    return false;
  } else {  // unknown functional!!
    name = "Illegal Exchange-Correlation Functional!";
    return false;
  }
  return false;
}
