...
// getvmt.f in LSMS 1
// vshift still needs to be calculated somewhere else first! (now local and
// =0.0)
void getvmt(LSMSSystemParameters &lsms, AtomData &atom,
   ...
   * Coulomb energy. Lattice constant factor is included in madmat
   */
  ...
  switch (lsms.mtasa) {
    case lsms::PotentialType::MT:  // Muffin-Tin case
  ...
  switch (lsms.mtasa) {
    ...
    default:  // Muffin-Tin case
      ...
      vmt = vmt1 * atom.omegaMT +
            surfaceAreaMT * (atom.omegaMT * atom.rhoInt / 5.0 + qsub[mytype]);
      u0 += atom.rhoInt * atom.omegaMT *
            (-6.0 / 5.0 * atom.rhoInt * atom.omegaMT + 3.0 * qsub[mytype]) /
            rSphere;
      ...
      if (lsms.mtasa >= 2)  // MT-ASA case
      {
        ...
        Real qCorrection = atom.qtotws - atom.qtotmt -
                           atom.rhoInt * (atom.omegaWS - atom.omegaMT);
