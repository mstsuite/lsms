...
*/
    if (lsms.global.iprint > 0) {
      ...
      printf(" LOCAL Moment orientation = (%18.11f, %18.11f, %18.11f)\n", local.atom[i].evecNew[0],
             local.atom[i].evecNew[1], local.atom[i].evecNew[2]);
    }
    ...
    switch (chargeSwitch) {
      case 1: {
        ...
        if (lsms.global.iprint > 0) {
          printf("Spin = 1 rhotot[jmt-1], [jmt], [jmt+1] = %20.16f %20.16f %20.16f\n",
                 ...
                 local.atom[i].rhotot(local.atom[i].jmt + 1, 0));
          printf("Spin = 2 rhotot[jmt-1], [jmt], [jmt+1] = %20.16f %20.16f %20.16f\n",
                 local.atom[i].rhotot(local.atom[i].jmt - 1, lsms.n_spin_pola - 1),
                 local.atom[i].rhotot(local.atom[i].jmt, lsms.n_spin_pola - 1),
                 local.atom[i].rhotot(local.atom[i].jmt + 1, lsms.n_spin_pola - 1));
        }

        getqm_mt_(&lsms.n_spin_pola, &local.atom[i].jmt, &local.atom[i].rInscribed, rTemp, &local.atom[i].rhotot(0, 0),
                  &lsms.global.iprpts, &rhoTemp(0, 0, i), &lsms.mtasa, &local.atom[i].qtotmt, &local.atom[i].mtotmt,
                  &rSphere, &lsms.global.iprint);
      ...
      default: {
        ...
        if (local.atom[i].rhoNew(local.atom[i].jmt, lsms.n_spin_pola - 1) == 0.0)
          local.atom[i].rhoNew(local.atom[i].jmt, lsms.n_spin_pola - 1) = local.atom[i].rhoNew(local.atom[i].jmt - 1,
                                                                                               lsms.n_spin_pola - 1);
        if (local.atom[i].rhoNew(local.atom[i].jmt + 1, lsms.n_spin_pola - 1) == 0.0)
          local.atom[i].rhoNew(local.atom[i].jmt + 1, lsms.n_spin_pola - 1) = local.atom[i].rhoNew(
              local.atom[i].jmt - 1, lsms.n_spin_pola - 1);

        if (lsms.global.iprint > 0) {
          printf("Spin = 1 rhoNew[jmt-1], [jmt], [jmt+1] = %20.16f %20.16f %20.16f\n",
                 local.atom[i].rhoNew(local.atom[i].jmt - 1, 0), local.atom[i].rhoNew(local.atom[i].jmt, 0),
                 local.atom[i].rhoNew(local.atom[i].jmt + 1, 0));
          printf("Spin = 2 rhoNew[jmt-1], [jmt], [jmt+1] = %20.16f %20.16f %20.16f\n",
                 local.atom[i].rhoNew(local.atom[i].jmt - 1, lsms.n_spin_pola - 1),
                 local.atom[i].rhoNew(local.atom[i].jmt, lsms.n_spin_pola - 1),
                 local.atom[i].rhoNew(local.atom[i].jmt + 1, lsms.n_spin_pola - 1));
        }

        getqm_mt_(&lsms.n_spin_pola, &local.atom[i].jmt, &local.atom[i].rInscribed, rTemp, &local.atom[i].rhoNew(0, 0),
                  &lsms.global.iprpts, &rhoTemp(0, 0, i), &lsms.mtasa, &local.atom[i].qtotmt, &local.atom[i].mtotmt,
                  &rSphere, &lsms.global.iprint);
      }
    }
...
/*
     ...
     ----------------------------------------------------------------
*/
    Real rSphere; // r_getqm in genpot_c.f
    int jmt;

    switch (lsms.mtasa) {
      case 1:
        rSphere = local.atom[i].rws; // rmt in genpot
        jmt = local.atom[i].jws;
        break;
      case 2:
        rSphere = local.atom[i].rws; // rmt in genpot
      ...
      case 1: {
        ...
        if (lsms.global.iprint > 0) {
          printf("Spin = 1 rhotot[jmt-1], [jmt], [jmt+1] = %20.16f %20.16f %20.16f\n", local.atom[i].rhotot(jmt - 1, 0),
                 local.atom[i].rhotot(jmt, 0), local.atom[i].rhotot(local.atom[i].jmt + 1, 0));
          printf("Spin = 2 rhotot[jmt-1], [jmt], [jmt+1] = %20.16f %20.16f %20.16f\n",
                 local.atom[i].rhotot(jmt - 1, lsms.n_spin_pola - 1), local.atom[i].rhotot(jmt, lsms.n_spin_pola - 1),
                 local.atom[i].rhotot(jmt + 1, lsms.n_spin_pola - 1));
        }

        getqm_mt_(&lsms.n_spin_pola, &jmt, &local.atom[i].rInscribed, rTemp, &local.atom[i].rhotot(0, 0),
                  &lsms.global.iprpts, &rhoTemp(0, 0, i), &lsms.mtasa, &local.atom[i].qtotmt, &local.atom[i].mtotmt,
                  &rSphere, &lsms.global.iprint);
      ...
      default: {
        ...
        if (lsms.global.iprint > 0) {
          ...
          printf("Spin = 2 rhoNew[jmt-1], [jmt], [jmt+1] = %20.16f %20.16f %20.16f\n",
                 ...
                 local.atom[i].rhoNew(jmt + 1, lsms.n_spin_pola - 1));
        }
...
*/
  ...
  switch (lsms.mtasa) {
    case 1:             // ASA (not implemented)
      ...
      for (int i = 0; i < local.num_local; i++) {
        Real rSphere = local.atom[i].rws; // rmt in genpot
        int jmt = local.atom[i].jws;

        std::vector<Real> rTemp(jmt + 3);
        ...
        if (chargeSwitch == 1) {
          for (int is = 0; is < lsms.n_spin_pola; is++)
            for (int ir = 0; ir < local.atom[i].jws; ir++)
              local.atom[i].rhotot(ir, is) +=
                  local.atom[i].rhoInt * local.atom[i].r_mesh[ir] * local.atom[i].r_mesh[ir];
          ...
          getqm_mt_(&lsms.n_spin_pola, &jmt, &local.atom[i].rInscribed, &rTemp[0], &local.atom[i].rhotot(0, 0),
                    ...
                    &local.atom[i].qtotmt, &local.atom[i].mtotmt, &rSphere, &lsms.global.iprint);
        } else {
          for (int is = 0; is < lsms.n_spin_pola; is++)
            for (int ir = 0; ir < local.atom[i].jws; ir++)
              local.atom[i].rhoNew(ir, is) +=
                  local.atom[i].rhoInt * local.atom[i].r_mesh[ir] * local.atom[i].r_mesh[ir];

          if (local.atom[i].rhoNew(jmt + 1, 0) == 0.0)
        ...
        local.atom[i].qInt = 0.0;
        local.atom[i].rhoInt = 0.0;
        //}
  ...
  if (lsms.global.iprint >= 1) {
    for (int i = 0; i < local.num_local; i++) {
      ...
      for (int j = 0; j < crystal.num_types; j++) {
        printf("%5d %25.15f %25.15f %25.15f\n", j, qsub[j], local.atom[i].madelungMatrix[j],
               qsub[i] * local.atom[i].madelungMatrix[j]);
    ...
    case 1:                            // ASA case
      ...
      for (int i = 0; i < local.num_local; i++) {
        ...
        */
        int i_vdif = 0;
        if (chargeSwitch == 1) {
          calculate_asa_ro3_(&lsms.n_spin_pola, &local.atom[i].rhotot(0, 0),
                             &local.atom[i].rhotot(0, lsms.n_spin_pola - 1),
                             &i_vdif, &local.atom[i].r_mesh[0], &jmt, &local.atom[i].rmt, &lsms.global.iprpts,
                             &ro3[i], &dz[i]);
        } else {
          calculate_asa_ro3_(&lsms.n_spin_pola, &local.atom[i].rhoNew(0, 0),
...
*/
        ...
        switch (chargeSwitch) {
          case 1: {
            newexchg_(&lsms.n_spin_pola, &spin, &local.atom[i].rhotot(0, 0),
                      &local.atom[i].rhotot(0, lsms.n_spin_pola - 1),
...
//                                              &local.atom[i].exchangeCorrelationE, &local.atom[i].exchangeCorrelationV[0]);
          ...
          lsms.exch_corr->evaluate(rhoLocal, local.atom[i].exchangeCorrelationE, local.atom[i].exchangeCorrelationV);
...
// write the xc potential, xc energy density, and rho for the first atom:
    if (false) {
      if (i == 0) {
        ...
        printf("exchangeCorrelationV[] = %lg %lg\n", local.atom[i].exchangeCorrelationV[0],
               local.atom[i].exchangeCorrelationV[1]);
  ...
  if (lsms.mtasa >= 1) {
    ...
    if (lsms.mtasa == 1) {
      for (int i = 0; i < local.num_local; i++) {
        vxout[0] += local.atom[i].exchangeCorrelationV[0] * local.n_per_type[i];
        vxout[1] += local.atom[i].exchangeCorrelationV[1] * local.n_per_type[i];
      }
    } else {
      for (int i = 0; i < local.num_local; i++) {
        // vxout[0] += local.atom[i].exchangeCorrelationV[0]*local.n_per_type[i]*qint1;
        // vxout[1] += local.atom[i].exchangeCorrelationV[1]*local.n_per_type[i]*qint1;
      }
      printf("MTASA = 2 not implemented!\n");
      exit(1);
    }

    globalSum(comm, vxout, 2);

    vxout[0] = vxout[0] / Real(lsms.num_atoms);
    vxout[1] = vxout[1] / Real(lsms.num_atoms);
    if (lsms.mtasa == 1) {
      for (int i = 0; i < local.num_local; i++) {
        local.atom[i].exchangeCorrelationV[0] = vxout[0];
        local.atom[i].exchangeCorrelationV[1] = vxout[1];
      }
    } else {
      for (int i = 0; i < local.num_local; i++) {
        // local.atom[i].exchangeCorrelationV[0] = vxout[0]/qint1_total;
        // local.atom[i].exchangeCorrelationV[1] = vxout[1]/qint1_total;
      ...
      switch (chargeSwitch) {
        case 1: {
          newpot_(&lsms.n_spin_pola, &local.atom[i].ztotss, &local.atom[i].rhotot(0, 0),
                  ...
                  &local.atom[i].exchangeCorrelationPotential(0, is), &vmt1[i], &vmt,
                  &local.atom[i].exchangeCorrelationV[is], rTemp,
...
*/
  ...
  for (int i = 0; i < local.num_local; i++) {
    switch (chargeSwitch) {
      case 1: {
        local.atom[i].vdif =
            local.atom[i].exchangeCorrelationV[lsms.n_spin_pola] - local.atom[i].exchangeCorrelationV[0];
