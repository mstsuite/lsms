...
extern "C" {
void zblock_lu_(Complex *a, int *lda, int *blk_sz, int *nblk, int *ipvt,
                int *mp, int *idcol, int *k);
void zblock_lu_cuda_c_(Complex *a, int *lda, int *blk_sz, int *nblk, int *ipvt,
                       int *mp, int *idcol, int *k);
}

int zblock_lu_cpp(Matrix<Complex> &a, int *blk_sz, int nblk, int *ipvt,
                  int *idcol);

#if defined(ACCELERATOR_CUBLAS)
int zblock_lu_cublas(cublasHandle_t handle, Matrix<Complex> &a, int *blk_sz,
                     int nblk, int *ipvt, int *idcol);
#endif
...
void block_inverse(Matrix<Complex> &a, int *blk_sz, int nblk,
  ...
  for (int i = 0; i < blk_sz[0]; i++)
    for (int j = 0; j < blk_sz[0]; j++) a(j, i) = 0.0;

#if defined(ACCELERATOR_CUBLAS)
  zblock_lu_cublas(DeviceStorage::getCublasHandle(), a, blk_sz, nblk, ipvt,
                   idcol);
  ...
  for (int i = 0; i < blk_sz[0]; i++)
    for (int j = 0; j < blk_sz[0]; j++) delta(j, i) = -a(j, i);
}
