...
__device__ inline void calculateSinCosPowersHip(Real *rij, int lend,
                                                Real *sinmp, Real *cosmp) {
...
__device__ void associatedLegendreFunctionNormalizedHip(Real x, int lmax,
                                                        Real *Plm) {
  ...
  for (int m = 1; m <= lmax; m++) {
    ...
    // Plm[plmIdxDev(m,m)] = - std::sqrt(Real(2*m+1)/Real(2*m)) * y *
    // Plm[plmIdxDev(m-1,m-1)];
    Plm[plmIdxDev(m, m)] =
        -sqrt(Real(2 * m + 1) / Real(2 * m)) * y * Plm[plmIdxDev(m - 1, m - 1)];
    // \bar{P}_{mm-1} = \sqrt{2 m + 1} x \bar{P}_{m-1, m-1}
    // Plm[plmIdxDev(m,m-1)] = std::sqrt(Real(2*m+1)) * x *
    // Plm[plmIdxDev(m-1,m-1)];
    Plm[plmIdxDev(m, m - 1)] =
        sqrt(Real(2 * m + 1)) * x * Plm[plmIdxDev(m - 1, m - 1)];
  }
  ...
  for (int m = 0; m < lmax; m++) {
    for (int l = m + 2; l <= lmax; l++) {
      ...
      Real b_lm = sqrt(Real((l - 1) * (l - 1) - m * m) /
                       Real(4 * (l - 1) * (l - 1) - 1));
...
size_t sharedMemoryBGijHip(LSMSSystemParameters &lsms, size_t *hfnOffset,
...
__global__ void setBGijHip(bool fullRelativity, int n_spin_cant, int *LIZlmax,
                           int *offsets, size_t nrmat_ns,
                           deviceDoubleComplex *devBgij) {
    ...
    for (int ij = hipThreadIdx_x; ij < kkri * kkrj; ij += hipBlockDim_x) {
      ...
      */
      devBgij[IDX(iOffset + kkri + i, jOffset + j, nrmat_ns)] =
          make_hipDoubleComplex(0.0, 0.0);  // bgij(iOffset + i, jOffset + j);
      devBgij[IDX(iOffset + i, jOffset + kkrj + j, nrmat_ns)] =
          make_hipDoubleComplex(0.0, 0.0);  // bgij(iOffset + i, jOqffset + j);
...
__global__ void buildGijHipKernel(
    ...
    deviceDoubleComplex energy, deviceDoubleComplex prel, int *offsets,
    size_t nrmat_ns, deviceDoubleComplex *devBgij)
#else
    deviceDoubleComplex energy, deviceDoubleComplex prel, int *offsets,
    size_t nrmat_ns, deviceDoubleComplex *devBgij, char *testSM)
#endif
//  void buildBGijCPU(LSMSSystemParameters &lsms, AtomData &atom, int ir1, int
//  ir2, Real *rij,
//                  Complex energy, Complex prel, int iOffset, int jOffset,
//                  Matrix<Complex> &bgij)
{
  int ir1 = hipBlockIdx_x;
  int ir2 = hipBlockIdx_y;
  ...
  if (ir1 != ir2) {
    ...
    deviceDoubleComplex *hfn =
        (deviceDoubleComplex *)(sharedMemory + hfnOffset);
    ...
    // deviceDoubleComplex *dlm = (deviceDoubleComplex *) (sharedMemory +
    // dlmOffset);
    ...
    for(int l = hipThreadIdx_x; l<=lend; l += hipBlockDim_x)
    {
      ...
      {
        m1m = -m1m;
        deviceDoubleComplex fac = plm[ll+m] *
    make_hipDoubleComplex(cosmp[m],sinmp[m]); dlm[j-m] = hfn[l]*m1m*fac;
    ...
    */
    ...
    for (int ij = hipThreadIdx_x; ij < kkri * kkrj; ij += hipBlockDim_x) {
      ...
      // auto devBgijPointer = &devBgij[IDX(iOffset + lm2, jOffset + lm1,
      // nrmat_ns)];

      // devBgij[IDX(iOffset + lm2, jOffset + lm1, nrmat_ns)] =
      // make_hipDoubleComplex(0.0, 0.0); *devBgijPointer =
      // make_hipDoubleComplex(0.0, 0.0);
      auto devBgijValue = make_hipDoubleComplex(0.0, 0.0);
    ...
    //  __syncthreads();
    //  setBGijCuda(fullRelativity, n_spin_cant, LIZlmax, ir1, ir2, iOffset,
    //  jOffset, nrmat_ns, devBgij);
  }
}

__device__ void buildTmatNHip(int ispin, int n_spin_pola, int n_spin_cant,
                              int iie, int blkSizeTmatStore, int tmatStoreLDim,
                              int kkr1, int kkr2, int lizStoreIdx,
                              deviceDoubleComplex *devTmatStore, int kkrsz_ns,
                              deviceDoubleComplex *tmat_n) {
  // Matrix<Complex> tmat_n(lsms.n_spin_cant*atom.kkrsz,
  // lsms.n_spin_cant*atom.kkrsz);
  if (hipThreadIdx_x == 0) {
    int im = 0;
    if (n_spin_pola == n_spin_cant)  // non polarized or spin canted
    {
      ...
      for (int js = 0; js < n_spin_cant; js++) {
        ...
        for (int j = 0; j < kkr1; j++) {
          for (int is = 0; is < n_spin_cant; is++) {
            ...
            int tmatIdx0 =
                IDX(iie * blkSizeTmatStore + jm, lizStoreIdx, tmatStoreLDim);
            for (int i = 0; i < kkr1; i++) {
              // tmat_n[im+i] = devTmatStore[IDX(iie*blkSizeTmatStore+jm+i,
              // lizStoreIdx, tmatStoreLDim)];
      ...
      // int ispin=0;
      printf(
          "warning: cant't test building kkrMatrix for collinear spin "
          "polarized yet!\n");
      // exit(1);
...
__global__ void buildKKRMatrixMultiplyKernelHip(
  ...
  if (ir1 != ir2) {
    ...
    //                     devAtom.LIZStoreIdx[ir1]), &kkr1_ns,
    //                     // &tmat_n(0, 0), &kkr1_ns,
    ...
    */
    ...
    //      for(int j=0; j<kkr2_ns; j++)
    //    buildTmatNCuda(ispin, n_spin_pola, n_spin_cant, iie, blkSizeTmatStore,
    //    tmatStoreLDim,
    //                   kkr1, kkr2, LIZStoreIdx[ir1], devTmatStore, kkrsz_ns,
    //                   tmat_n);
    ...
    for (int ij = hipThreadIdx_x; ij < kkr1_ns * kkr2_ns; ij += hipBlockDim_x) {
      ...
      // devM[IDX(iOffset + i, jOffset + j, nrmat_ns)] =
      // make_hipDoubleComplex(0.0,0.0); *devMPointer =
      // make_hipDoubleComplex(0.0,0.0);
...
void buildKKRMatrixLMaxIdenticalHip(LSMSSystemParameters &lsms,
  ...
  deviceDoubleComplex cuEnergy =
      make_hipDoubleComplex(energy.real(), energy.imag());
  ...
  size_t smSize = sharedMemoryBGijHip(lsms, &hfnOffset, &sinmpOffset,
  ...
  buildGijHipKernel<<<blocks, threads, smSize>>>(
  ...
  setBGijHip<<<blocks, threads>>>(fullRelativity, lsms.n_spin_cant,
...
// deviceMemcpy(testDlm, devTestSM + dlmOffset, AngularMomentumIndices::ndlj*sizeof(Complex), deviceMemcpyDeviceToHost);
  ...
  /*
    ...
    for(int ir1 = 0; ir1 < atom.numLIZ; ir1++)
    {
      int iOffset = ir1 * kkrsz_ns; // this assumes that there are NO lStep
    reductions of lmax!!! for(int ir2 = 0; ir2 < atom.numLIZ; ir2++)
      {
        int jOffset = ir2 * kkrsz_ns; // this assumes that there are NO lStep
    reductions of lmax!! int lmax1 = atom.LIZlmax[ir1]; int lmax2 =
    atom.LIZlmax[ir2]; int kkr1=(lmax1+1)*(lmax1+1); int
    kkr2=(lmax2+1)*(lmax2+1); int kkr1_ns = 2*kkr1; int kkr2_ns = 2*kkr2; int
    nrel_rel=0; if(lsms.relativity==full) nrel_rel=1;
    ...
    for(int i=0; i<2*kkri; i++)
      for(int j=0; j<2*kkrj; j++)
      {
        ...
        {
          printf("buildBGijCPU  [idx=%d]: bgij(%d + %d, %d + %d) [%g + %gi] !=
    bgijTest(%d, %d) [%g + %gi]\n", idx, iOffset, i, jOffset, j, bgij(iOffset +
    i, jOffset + j).real(), bgij(iOffset + i, jOffset + j).imag(), i, j,
    bgijTest(i,j).real(), bgijTest(i,j).imag()); exitCompare = true;
        }
        idx++;
      }
    if(exitCompare) exit(1);
    ...
    if((ir1==1 && ir2==0) || (ir1==10 && ir2==0))
    {
      printf("ir1=%d, ir2=%d: bgij(0,0) = %g + %gi; bgijTest(0,0) = %g + %gi\n",
             ir1, ir2, bgij(0,0).real(), bgij(0,0).imag(), bgijTest(0,0).real(),
    bgijTest(0,0).imag()); printf("    rij = %g %g %g;  prel=%g + %gi\n",
    rij[0],  rij[1], rij[2], prel.real(), prel.imag()); printf("    kkr1 = %d;
    kkr2 = %d; kkrsz = %d\n", kkr1, kkr2, kkrsz);
    }
  */
  ...
  {
    int iOffset = ir1 * kkrsz_ns; // this assumes that there are NO lStep
reductions of lmax!!! for(int ir2 = 0; ir2 < devAtom.numLIZ; ir2++)
    {
      if(ir1 != ir2)
      {
        int jOffset = ir2 * kkrsz_ns; // this assumes that there are NO lStep
reductions of lmax!!! int lmax1 = devAtom.LIZlmax[ir1]; int lmax2 =
devAtom.LIZlmax[ir2]; int kkr1=(lmax1+1)*(lmax1+1); int
kkr2=(lmax2+1)*(lmax2+1); int kkr1_ns = kkr1 * lsms.n_spin_cant; int kkr2_ns =
kkr2 * lsms.n_spin_cant;

        // buildBGijCuda(lsms, atom, ir1, ir2, rij, energy, prel, iOffset,
jOffset, bgij);
        // buildBGijCPU(lsms, atom, ir1, ir2, rij, energy, prel, 0, 0,
bgijSmall);
...
local.tmatStore(iie*local.blkSizeTmatStore + , atom.LIZStoreIdx[ir1]) *
...
void buildKKRMatrixHip(LSMSSystemParameters &lsms, LocalTypeInfo &local,
  ...
  if (lmaxIdentical) {
    ...
    buildKKRMatrixLMaxIdenticalHip(lsms, local, atom, devStorage, devAtom,
                                   ispin, iie, energy, prel, devM);
  } else {
    // printf("lmax not identical in buildKKRMatrix\n");
    buildKKRMatrixLMaxDifferentHip(lsms, local, atom, devStorage, devAtom,
                                   ispin, iie, energy, prel, devM);
  }
}
