...
void solveTau00SpinCanted_zgesv(LSMSSystemParameters &lsms,
  ...
  int ipiv[mSize];
  int info;
  LAPACK::zgesv_(&mSize, &blockSize, &m(0, 0), &mSize, ipiv, &tau(0, 0), &mSize,
                 &info);

  // copy result into tau00
  for (int i = 0; i < blockSize; i++)
    for (int j = 0; j < blockSize; j++) tau00(i, j) = tau(i, j);
}

void solveTau00Collinear_zgesv(LSMSSystemParameters &lsms, LocalTypeInfo &local,
                               AtomData &atom, int spin, Matrix<Complex> &tau00,
                               Matrix<Complex> &m) {
