...
void setBGijCPU(LSMSSystemParameters &lsms, AtomData &atom, int ir1, int ir2,
                int iOffset, int jOffset, Matrix<Complex> &bgij) {
  ...
  if (lsms.relativity != full) {
    for (int i = 0; i < kkri; i++)
      for (int j = 0; j < kkrj; j++) {
        bgij(iOffset + kkri + i, jOffset + j) =
            0.0;  // bgij(iOffset + i, jOffset + j);
        bgij(iOffset + i, jOffset + kkrj + j) =
            0.0;  // bgij(iOffset + i, jOffset + j);
        bgij(iOffset + kkri + i, jOffset + kkrj + j) =
            bgij(iOffset + i, jOffset + j);
      }
  } else {
    /*
...
void buildBGijCPU(LSMSSystemParameters &lsms, AtomData &atom, int ir1, int ir2,
                  Real *rij, Complex energy, Complex prel, int iOffset,
                  int jOffset, Matrix<Complex> &bgij) {
  ...
  for (int i = 0; i < kkri; i++)
    for (int j = 0; j < kkrj; j++) bgij(iOffset + i, jOffset + j) = 0.0;
    ...
    for (int lm2 = 0; lm2 < kkri; lm2++) {
      ...
      */
      ...
      for (int l3 = l1 + l2; l3 >= llow; l3 -= 2) {
        ...
        bgij(iOffset + lm2, jOffset + lm1) +=
            GauntCoeficients::cgnt(l3 / 2, lm1, lm2) * dlm[j];
      }
...
void buildKKRMatrixLMaxIdenticalCPU(LSMSSystemParameters &lsms,
  ...
  for (int ir1 = 0; ir1 < atom.numLIZ; ir1++) {
    ...
    for (int ir2 = 0; ir2 < atom.numLIZ; ir2++) {
      if (ir1 != ir2) {
        ...
        buildBGijCPU(lsms, atom, ir1, ir2, rij, energy, prel, iOffset, jOffset,
                     bgij);
          ...
          for (int j = 0; j < 2 * kkrj; j++) {
            ...
            // local.tmatStore(iie*local.blkSizeTmatStore+idx,atom.LIZStoreIdx[ir1]))
            {
              printf(
                  ...
                  local
                      .tmatStore(iie * local.blkSizeTmatStore + jm,
                                 atom.LIZStoreIdx[ir1])
                      .real(),
                  local
                      .tmatStore(iie * local.blkSizeTmatStore + jm,
...
void buildKKRMatrixLMaxDifferentCPU(LSMSSystemParameters &lsms,
                                    LocalTypeInfo &local, AtomData &atom,
                                    int ispin, int iie, Complex energy,
                                    Complex prel, Matrix<Complex> &m) {
  int nrmat_ns = lsms.n_spin_cant * atom.nrmat;  // total size of the kkr matrix
...
void buildKKRMatrixCPU(LSMSSystemParameters &lsms, LocalTypeInfo &local,
  ...
  if (lmaxIdentical) {
    ...
    buildKKRMatrixLMaxIdenticalCPU(lsms, local, atom, ispin, iie, energy, prel,
                                   m);
  } else {
    // printf("lmax not identical in buildKKRMatrix\n");
    buildKKRMatrixLMaxDifferentCPU(lsms, local, atom, ispin, iie, energy, prel,
                                   m);
  }
}
