...
static void buildKKRSizeTMatrix(LSMSSystemParameters &lsms,
...
// given the  m-Matrix [(blockSize*numBlocks) x (blockSize*numBlocks)] and
// tMatrix[0] [blockSize x blockSize] calculate the t00-Matrix as the blockSize
// x blockSize diagonal block of  (1 - tG)^-1 t
void solveTau00zgesv(LSMSSystemParameters &lsms, LocalTypeInfo &local,
  ...
  for (int i = 0; i < kkrsz_ns; i++)
    for (int j = 0; j < kkrsz_ns; j++) tau00(i, j) = tau(i, j);
}
...
extern "C" {
void block_inv_(Complex *a, Complex *vecs, int *lda, int *na, int *mp,
                int *ipvt, int *blk_sz, int *nblk, Complex *delta, int *iwork,
                double *rwork, Complex *work1, int *alg, int *idcol,
                int *iprint);
}
...
void solveTau00zblocklu_f77(LSMSSystemParameters &lsms, LocalTypeInfo &local,
  ...
  for (int i = 0; i < kkrsz_ns; i++)
    for (int j = 0; j < kkrsz_ns; j++) wbig(i, j) -= delta(i, j);
  //  c     ================================================================
