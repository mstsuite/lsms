...
void transferT0MatrixToGPUHip(Complex *devT0, LSMSSystemParameters &lsms, LocalTypeInfo &local, AtomData &atom, int iie)
{
  ...
  hipMemcpy(devT0, &local.tmatStore(iie*local.blkSizeTmatStore,atom.LIZStoreIdx[0]),
...
__global__ void copyTMatrixToTauHip(hipDoubleComplex *tau, hipDoubleComplex *t, int kkrsz, int nrmat)
{
  int i = hipBlockIdx_x*hipBlockDim_x + hipThreadIdx_x;
  if(i < kkrsz)
  {
