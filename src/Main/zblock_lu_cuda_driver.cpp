...
void *deviceStorage;
//#endif
extern "C" void zblock_lu_cuda_c_(std::complex<double> *, int *, int *, int *,
                                  int *, int *, int *, int *);
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < local.num_local; i++)
    deviceConstants[i] = allocateDConst();
  // deviceStorage.resize(lsms.global.GPUThreads);
  // for(int i=0; i<lsms.global.GPUThreads; i++)
  // deviceStorage[i]=allocateDStore();
  ...
  if (nblk == 1)
    ...
    int i = 1;
    for (i; i <= rem; i++) blk_sz[i] = min_sz + 1;
    for (i; i < nblk; i++) blk_sz[i] = min_sz;
  }
  ...
  int alg = 2;
  if (alg > 2)
    vecs = (Complex *)malloc((nrmat_ns * (kkrsz_ns * 6 + 6)) * sizeof(Complex));
...
#endif
  {
    ...
    zblock_lu_cuda_c_(&m(0, 0), &nrmat_ns, blk_sz, &nblk, ipvt, &nrmat_ns,
                      idcol, &k);
  }
#ifdef _OPENMP
