...
int main(int argc, char *argv[]) {
  ...
  bool restrict_steps{false};  // or the max. number of steps?
  bool restart{false};         // is it a restarted simulation?
  ...
  double energy_accumulator{
      0.0};  // accumulates the energy to calculate the mean
  ...
  for (int i = 0; i < argc; i++) {
    ...
    if (!strcmp("-num_lsms_per_window", argv[i]))
      num_lsms_per_window = atoi(argv[++i]);
    if (!strcmp("-num_steps", argv[i])) {
      num_steps = atoi(argv[++i]);
      restrict_steps = true;
    }
    if (!strcmp("-exchange_frequency", argv[i]))
      exchangeFrequency = atoi(argv[++i]);
    if (!strcmp("-walltime", argv[i])) {
      max_time = 60.0 * atof(argv[++i]);
      restrict_time = true;
    }
    if (!strcmp("-restart", argv[i])) restart = true;
...
#define SVN_REV "unknown"
  ...
  if (world_rank == 0) {
    ...
    std::cout << " Number of Wang-Landau energy windows = " << num_window
              << std::endl;
  ...
  if (num_lsms == 1) {
    ...
    // LSMS lsms_calc(MPI_COMM_WORLD, i_lsms_name, "1_", potentialShifter);
    LSMS lsms_calc(MPI_COMM_WORLD, i_lsms_name, "1_");

    potentialShifter.vSpinShiftFlag = lsms_calc.vSpinShiftFlag();
    potentialShifter.minShift = lsms_calc.potentialMinShift();
    potentialShifter.maxShift = lsms_calc.potentialMaxShift();
    ...
    if (energyCalculationMode == OneStepEnergy)
      ...
      std::cout << "multi-step Energy = " << lsms_calc.multiStepEnergy()
                << std::endl;
    else if (energyCalculationMode == ScfEnergy)
      std::cout << "self-consistent Energy = " << lsms_calc.scfEnergy()
                << std::endl;
    else {
    ...
    REWL *rewl{};
    if (rank == 0)
      rewl = new REWL(WLwalkersComm, num_window, num_lsms_per_window,
                      seeds[my_group]);
    ...
    // LSMS lsms_calc(local_comm, i_lsms_name, prefix, potentialShifter);
    ...
    for (int i = 0; i < 3 * size_lsms; i++) evecs[0][i] = 0.0;
    // YingWai's temp. fix  (Sep 2, 14):
    // The if-statement causes null pointer dereference when vSpinShiftFlag =
    // false
    // if (potentialShifter.vSpinShiftFlag) {
    vSpinShifts = (double *)malloc(sizeof(double) * size_lsms);
    for (int i = 0; i < size_lsms; i++) vSpinShifts[i] = 0.0;
    evecsAndSpinShifts = (double *)malloc(sizeof(double) * 4 * size_lsms);
    //}

    char *wl_inf{};
    char *wl_outf{};
    // Ying Wai's note:          (Apr 30, 14)
    // To read from the corresponding WLrestart file,
    // the name of wl_inf needs to be reconstructed for different walker (using
    // my_group)
    if (restart) {
      char restartFile[51];
    ...
    if (world_rank == 0) {
      ...
      std::cout << prefix << "executing LSMS(C++) for " << lsms_calc.numSpins()
                << " atoms\n";
    ...
    if (rank == 0) {
      ...
      switch (evecGenerationMode) {
        case Random:
          ...
          break;
        case Constant:
          generator = new ConstantEvecGenerator(size_lsms, ev0, 1);
          break;
        case WangLandau_1d:
          generator = new WL1dEvecGenerator<std::mt19937>(
              size_lsms, 1, num_window, num_lsms_per_window, my_group, evecs,
              potentialShifter, vSpinShifts, wl_inf, wl_outf, wl_stepf);
          break;
        case ExhaustiveIsing:
          // YingWai: need to check if num_lsms should be changed to 1    (Nov
          // 12, 14)
      ...
      std::cout << "starting main calculation in group " << my_group
                << std::endl;

      if (potentialShifter.vSpinShiftFlag)
        generator->initializeEvecAndPotentialShift(my_group, evecs[0],
                                                   vSpinShifts);
      else
        generator->initializeEvec(my_group, evecs[0]);

      // Ying Wai's note           (Feb 4, 14)
      // Missing here: see if the energy of the state is within the energy range
      // if not, generate a new state until it is
    ...
    /*
       ...
       5: calculate energy
          recognized energy calculation modes:
           OneStepEnergy :
               calclulate frozen potential band energy in one step (don't
       converge Ef) use only if the Fermi energy will not change due to MC
       steps! The only method available in LSMS_1.9 MultiStepEnergy : calculate
       frozen potential band energy after converging Fermi energy This should be
       the new default method if the Fermi energy doesn't change multiStepEnergy
       only performs one step and should be equivalent to oneStepEnergy The
       tolerance for Ef convergence can be set with LSMS::setEfTol(Real). The
       default tolerance is set in the LSMS::LSMS constructor
       (currently 1.0e-6). The maximum number of steps is read from the LSMS
       input file 'nscf' parameter. ScfEnergy : calculate the selfconsistent
       total energy. The maximum number of steps is read from the LSMS input
       file 'nscf' parameter.
    ...
    while (more_work) {
      switch (op) {
        case 5:
          ...
          if (potentialShifter.vSpinShiftFlag) {
            // double* evecsAndSpinShifts {};
            // evecsAndSpinShifts = (double*) malloc(sizeof(double) * 4 *
            // size_lsms);
            for (int i = 0; i < size_lsms; i++) {
              evecsAndSpinShifts[4 * i + 0] = evecs[0][3 * i + 0];
              evecsAndSpinShifts[4 * i + 1] = evecs[0][3 * i + 1];
              evecsAndSpinShifts[4 * i + 2] = evecs[0][3 * i + 2];
              evecsAndSpinShifts[4 * i + 3] = vSpinShifts[i];
            }

            lsms_calc.setEvecAndSpinPotentialShift4(evecsAndSpinShifts);
          } else
          ...
          if (return_moments_flag) lsms_calc.getMag(evecs[0]);
      ...
      if (rank == 0) {
        ...
        // false - the proposed configuration is rejected, update with the old
        // config.
        bool newConfigAcceptance{false};
        // recentAcceptance :
        // keeps track of whether the most recent acceptance is from WL or RE
        // determines how the new state will be generated
        bool recentAcceptance{false};
        ...
        if (generator_needs_moment) {
          ...
          newConfigAcceptance =
              generator->determineAcceptance(0, energy, magnetization);
        } else
          newConfigAcceptance = generator->determineAcceptance(0, energy);

        recentAcceptance = newConfigAcceptance;

        // Update histogram, change gamma and check flatness
        if (potentialShifter.vSpinShiftFlag) {
          if (generator->updateHistogram(0, evecs[0], vSpinShifts,
                                         newConfigAcceptance))
            more_work = false;
        } else {
        ...
        if (MCstepCount % exchangeFrequency == 0) {
          ...
          if (replicaExchangeAcceptance) {
            energy = energyTemp;
            recentAcceptance = replicaExchangeAcceptance;
            ...
            if (potentialShifter.vSpinShiftFlag)
              rewl->swapPotentialShifts(vSpinShifts, size_lsms);
            ...
            if (generator_needs_moment) {
              ...
              if (potentialShifter.vSpinShiftFlag) {
                if (generator->updateHistogramFromRE(0, evecs[0], energy,
                                                     magnetization, vSpinShifts,
                                                     my_group))
                  more_work = false;
              } else {
                if (generator->updateHistogramFromRE(0, evecs[0], energy,
                                                     magnetization, my_group))
                  more_work = false;
              }
            } else {
              if (potentialShifter.vSpinShiftFlag) {
                if (generator->updateHistogramFromRE(0, evecs[0], energy,
                                                     vSpinShifts, my_group))
                  more_work = false;
              } else {
                if (generator->updateHistogramFromRE(0, evecs[0], energy,
                                                     my_group))
        ...
        // if (MCstepCount % exchangeFrequency == 0 &&
        // replicaExchangeAcceptance)
        //  recentAcceptance = replicaExchangeAcceptance;
        // else
        //  recentAcceptance = newConfigAcceptance;

        generator->generateEvec(0, evecs[0], recentAcceptance);
        if (potentialShifter.vSpinShiftFlag)
          generator->generatePotentialShift(0, vSpinShifts, recentAcceptance);
        ...
        // Determine if more work will be done; this will be broadcasted to all
        // later
        if (!more_work) op = 2;
        ...
        if (step_out_flag && newConfigAcceptance) {
          ...
          if (potentialShifter.vSpinShiftFlag) {
            for (int j = 0; j < 4 * size_lsms; j += 4)
              step_out_file << evecs[0][j] << "  " << evecs[0][j + 1] << "  "
                            << evecs[0][j + 2] << "  " << vSpinShifts[j]
                            << std::endl;
          } else {
  ...
  papi_values[hw_counters] =
      PAPI_get_real_cyc() -
      papi_real_cyc_0;  // real time counter value in clock cycles
  papi_values[hw_counters + 1] =
      PAPI_get_real_usec() -
      papi_real_usec_0;  // real time counter value in microseconds
  papi_values[hw_counters + 2] =
      PAPI_get_virt_cyc() -
      papi_virt_cyc_0;  // virtual time counter value in clock cycles
  papi_values[hw_counters + 3] =
      PAPI_get_virt_usec() -
      papi_virt_usec_0;  // virtual time counter values in microseconds
