...
#define SVN_REV "unknown"
  ...
  if(world_rank == 0)
  {
    ...
    std::cout << " Number of Wang-Landau energy windows = "<< num_window << std::endl;
  ...
  else
  {
    ...
    // The if-statement causes null pointer dereference when vSpinShiftFlag = false
    ...
    if (rank == 0)
    {
      ...
      {   
        ...
        case Constant :
             ...
             break;
        case WangLandau_1d :
             generator = new WL1dEvecGenerator<std::mt19937>(size_lsms, 1, num_window, num_lsms_per_window, my_group, evecs, potentialShifter, vSpinShifts, wl_inf, wl_outf, wl_stepf);
             break;
        case ExhaustiveIsing :
             //YingWai: need to check if num_lsms should be changed to 1    (Nov 12, 14)
        ...
        case WangLandau_2d :
             ...
             break;
        default :
             std::cerr << "The code should never arrive here: UNKNOWN EVEC GENERATION MODE\n";
             exit(1);
      }
  
      // Generate the initial configuration
      std::cout << "starting main calculation in group " << my_group << std::endl;    
  
      if (potentialShifter.vSpinShiftFlag) 
        generator -> initializeEvecAndPotentialShift(my_group, evecs[0], vSpinShifts);
      else 
    ...
    */
    ...
    while (more_work)
    {
      ...
      if (rank == 0) 
      {
        ...
        if (potentialShifter.vSpinShiftFlag) {
          if (generator -> updateHistogram(0, evecs[0], vSpinShifts, newConfigAcceptance))
            more_work = false;
        }
        else {
        ...
        if (MCstepCount % exchangeFrequency == 0) 
        {
          ...
          if (replicaExchangeAcceptance)
          {
            ...
            if (generator_needs_moment)
            {
              ...
              if (potentialShifter.vSpinShiftFlag) {
                if (generator -> updateHistogramFromRE(0, evecs[0], energy, magnetization, vSpinShifts, my_group))
                  more_work = false;
              } 
              else {
              ...
              if (potentialShifter.vSpinShiftFlag) {
                if (generator -> updateHistogramFromRE(0, evecs[0], energy, vSpinShifts, my_group))
                  more_work = false;
              } 
              else {
          ...
          if (potentialShifter.vSpinShiftFlag) {
            for (int j=0; j<4*size_lsms; j+=4)
              step_out_file << evecs[0][j] << "  " << evecs[0][j+1] << "  " 
                            << evecs[0][j+2] << "  " << vSpinShifts[j] << std::endl;
          }
          else {
