...
class Graph1d {
 ...
 public:
  Graph1d()
      : delta(0.1), N(0), y(0), minX(std::numeric_limits<KeyType>::max()) {
    ;
  }
  Graph1d(KeyType _delta)
      : delta(_delta), N(0), y(0), minX(std::numeric_limits<KeyType>::max()) {
    ;
  }
  void setRangeAndClear(KeyType min, KeyType max, Int _Nval) {
    ...
    y.resize(N);
    minX = min;
    maxX = max;  // minY=maxY=ValueType(0);
  ...
  // This idx function calculates the index using minX as the reference,
  // it could give an index which is out of range
  // (used in WL-LSMS where the energy window grows with the discovery of a new
  // energy)
  inline Int idx(KeyType x) {
    return ((N == 0) ? -1 : Int(N * (x - minX) / (maxX - minX)));
  }
  // This idx_withinRange function calculates an index only if the input (x) is
  // within range [minX, maxX], otherwise it gives -1. (used in REWL where the
  // energy window does not grow)
  inline Int idx_withinRange(KeyType x) {
    if ((N == 0) || (x < minX) || (x >= maxX))
      return -1;
    else
      return (N * Int((x - minX) / (maxX - minX)));
  }

  inline KeyType keyFromIdx(Int i) {
    return minX + (KeyType(i) + KeyType(0.5)) * delta;
  }
  ...
  inline void extendTo(KeyType x) {
    ...
    if (N < 1) {
      ...
      if (i < 0) {
        ...
        N = N - i;
        minX += KeyType(i) * delta;
        i = 0;
      } else if (i >= N) {
        Int n = i - N + 1;
        y.resize(i + 1, ValueType(0));
        N = i + 1;
        maxX += KeyType(n) * delta;
      }
    }
  }
  ...
  inline ValueType getMinY() {
    ValueType h = std::numeric_limits<ValueType>::max();
    for (Int i = 0; i < N; i++)
      if (y[i] < h) h = y[i];
    return h;
  }
  inline ValueType getMinYMasked(std::vector<int> &mask) {
    ValueType h = std::numeric_limits<ValueType>::max();
    for (Int i = 0; i < N; i++)
      if (mask[i] && y[i] < h) h = y[i];
    return h;
  }
  inline ValueType getMaxY() {
    ValueType h = std::numeric_limits<ValueType>::min();
    for (Int i = 0; i < N; i++)
      if (y[i] > h) h = y[i];
    return h;
  }
  inline ValueType getMaxYMasked(std::vector<int> &mask) {
    ValueType h = std::numeric_limits<ValueType>::min();
    for (Int i = 0; i < N; i++)
      if (mask[i] && y[i] > h) h = y[i];
    return h;
  ...
  inline void getMinMaxYMasked(ValueType &hMin, ValueType &hMax,
                               std::vector<int> &mask) {
    hMin = std::numeric_limits<ValueType>::max();
    hMax = std::numeric_limits<ValueType>::min();
    for (Int i = 0; i < N; i++) {
  ...
  inline ValueType getMeanY() {
    ...
    for (Int i = 0; i < N; i++) h += y[i];
    return h / ValueType(N);
  }
  inline ValueType getMeanYMasked(std::vector<int> &mask) {
    ValueType h = ValueType(0);
    Int count = 0;
    for (Int i = 0; i < N; i++) {
      if (mask[i]) {
  ...
  inline void getMinMaxYInInterval(KeyType b, KeyType t, ValueType &hMin,
                                   ValueType &hMax) {
    hMin = std::numeric_limits<ValueType>::max();
    hMax = std::numeric_limits<ValueType>::min();
    for (Int i = idx(b); i <= idx(t); i++) {
  ...
  inline ValueType getMeanYInInterval(KeyType b, KeyType t) {
    ValueType h = ValueType(0);
    for (Int i = idx(b); i <= idx(t); i++) h += y[i];
    return h / ValueType(idx(t) - idx(b) + 1);
  }
  void scale(ValueType s) {
    for (Int i = 0; i < N; i++) y[i] *= s;
  }
  void clear() {
    for (Int i = 0; i < N; i++) y[i] = ValueType(0);
  }
};

template <class ValueType, class KeyType, class Int>
inline bool syncronizeGraphs(Graph1d<ValueType, KeyType, Int> &a,
                             Graph1d<ValueType, KeyType, Int> &b) {
  if (a.getDelta() != b.getDelta())
    return false;  // we cant sync if the intervals are different
  if (a.getMinX() < b.getMinX())
    b.extendTo(a.getMinX());
  else
    a.extendTo(b.getMinX());
  if (a.getMaxX() > b.getMaxX())
    b.extendTo(a.getMaxX());
  else
    a.extendTo(b.getMaxX());
...
class Kernel1d : public Graph1d<ValueType, KeyType, Int> {
 ...
 public:
  Kernel1d() { ; }
  Kernel1d(KeyType _delta, KeyType _width)
      : Graph1d<ValueType, KeyType, Int>(_delta) {
    width = _width;
    this->extendTo(KeyType(0));
    this->extendTo(width);
    this->extendTo(-width);
    center = this->idx(KeyType(0));
  }
...
inline ValueType gaussian(KeyType x) {
  return ValueType(std::sqrt(1.0 / (2.0 * M_PI))) *
         std::exp(ValueType(-0.5 * x * x));
}
...
void initEpanechnikov(Kernel1d<ValueType, KeyType, Int> &k) {
  ...
  for (Int i = 0; i < k.getN(); i++)
    k[i] = ValueType(l) * epanechnikov<ValueType, KeyType>(l * k.keyFromIdx(i));
}
...
void initQuartic(Kernel1d<ValueType, KeyType, Int> &k) {
  KeyType l = KeyType(1) / KeyType(k.getWidth());
  for (Int i = 0; i < k.getN(); i++)
    k[i] = ValueType(l) * quartic<ValueType, KeyType>(l * k.keyFromIdx(i));
}
...
void initTriWight(Kernel1d<ValueType, KeyType, Int> &k) {
  KeyType l = KeyType(1) / KeyType(k.getWidth());
  for (Int i = 0; i < k.getN(); i++)
    k[i] = ValueType(l) * triwight<ValueType, KeyType>(l * k.keyFromIdx(i));
}
...
void initTriangle(Kernel1d<ValueType, KeyType, Int> &k) {
  KeyType l = KeyType(1) / KeyType(k.getWidth());
  for (Int i = 0; i < k.getN(); i++)
    k[i] = ValueType(l) * triangle<ValueType, KeyType>(l * k.keyFromIdx(i));
}
...
void initUniform(Kernel1d<ValueType, KeyType, Int> &k) {
  KeyType l = KeyType(1) / KeyType(k.getWidth());
  for (Int i = 0; i < k.getN(); i++)
    k[i] = ValueType(l) * uniform<ValueType, KeyType>(l * k.keyFromIdx(i));
}
...
void initGaussian(Kernel1d<ValueType, KeyType, Int> &k) {
  KeyType l = KeyType(1) / KeyType(k.getWidth());
  for (Int i = 0; i < k.getN(); i++)
    k[i] = ValueType(l) * gaussian<ValueType, KeyType>(l * k.keyFromIdx(i));
}
...
void initCosine(Kernel1d<ValueType, KeyType, Int> &k) {
  KeyType l = KeyType(1) / KeyType(k.getWidth());
  for (Int i = 0; i < k.getN(); i++)
    k[i] = ValueType(l) * cosine<ValueType, KeyType>(l * k.keyFromIdx(i));
}
...
void initKernel(KernelType t, Kernel1d<ValueType, KeyType, Int> &k) {
  switch (t) {
    ...
    case Cosine:
      ...
      break;
    default:
      std::cerr << "Unknown kernel type in Graph1d!\n";
      exit(1);
  }
}
...
template <class ValueType, class KeyType, class Int>
inline void addKernel(Graph1d<ValueType, KeyType, Int> &g,
                      Kernel1d<ValueType, KeyType, Int> &k, KeyType x) {
