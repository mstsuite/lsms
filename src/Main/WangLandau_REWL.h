...
class WL1dEvecGenerator : public EvecGenerator {
 public:
  ...
  // bool updateHistogram(int instance, double *evecs, double energy, double
  // magnetization, bool accepted);
  bool updateHistogram(int instance, double *evecs, double *potentialShifts,
                       bool accepted);

  bool updateHistogramFromRE(int instance, double *evecs, double energy,
                             double magnetization, int check);
  bool updateHistogramFromRE(int instance, double *evecs, double energy,
                             int check);
  bool updateHistogramFromRE(int instance, double *evecs, double energy,
                             double *potentialShifts, int check);
  bool updateHistogramFromRE(int instance, double *evecs, double energy,
                             double magnetization, double *potentialShifts,
                             int check);
  ...
  void initializeEvecAndPotentialShift(int instance, double *evecs,
 ...
 private:
  ...
  // rnd11(-1.0,1.0),rnd0pi(0.0,2.0*M_PI);
...
WL1dEvecGenerator<RNG>::WL1dEvecGenerator(
    int num_spins, int num_instances, int numWindows, int numInstancesPerWindow,
    int groupID, double **ev_p, PotentialShifter &potentialShifter,
    double *potentialShifts_p, const char *init_file_name,
    const char *out_file_name, const char *states_filename)
    : sw(states_filename),
      rnd01(0.0, 1.0),
      rnd11(-1.0, 1.0),
      rnd0pi(0.0, 2.0 * M_PI) {
  ...
  // * seed_seq is a sequence needed for generating random number seeds for
  // different walkers
  // * initialized with system time here, but will be overridden if seed is
  ...
  for (int i = 0; i < n_walkers; i++) {
    ...
    initializeEvecAndPotentialShift(i, evecs_pointer[i],
  ...
  */
  ...
  if (init_file_name != NULL && init_file_name[0] != 0) {
    ...
    if (json_root == NULL || json_root->type != JSON_OBJECT) {
      ...
      std::cerr << "WL1dEvecGenerator Constructor :: (" << init_file_name
    ...
    for (json_t *it = json_root->child; it != NULL; it = it->next) {
      ...
      // Choice 1: specify xMinFullRange, xMaxFullRange and overlap, subwindows
      // will be calculated automatically Choice 2: specify xMinForEachWindow
      // and xMaxForEachWindow (arrays), this overrides Choice 1
      if (label == "xMinFullRange")
        xMinFullRange = atof(it->child->text);
      else if (label == "xMaxFullRange")
        xMaxFullRange = atof(it->child->text);
      else if (label == "overlap")
        overlap = atof(it->child->text);
        ...
        if (j != numWindows) {
          std::cout << "ERROR #(xMin) " << j << " != numWindows " << numWindows
                    << std::endl;
          exit(1);
        }
      ...
      //*/
        ...
        if (j != n_spins) {
          std::cout << "ERROR #(potentialShifts) " << j << " != n_spins "
                    << n_spins << std::endl;
          exit(1);
        }
    ...
    // YingWai: need to communicate all the read-in data to different group
    // leaders    (Dec 17, 13) Just for testing to see if communication is OK:
    // int yingwai = 817;
  ...
  if (!readEnergyRangeForRestart) {
    ...
    if (!readLocalEnergyRange) {
      double energySubwindowWidth =
          (xMaxFullRange - xMinFullRange) /
          (1.0 + double(numWindows - 1) * (1.0 - overlap));
      xMin = xMinFullRange + floor(groupID / numInstancesPerWindow) *
                                 (1.0 - overlap) * energySubwindowWidth;
...
// YingWai: antiferromagnetic case should be set for walkers in the last window,
// not inst = 1 later ...     (Apr 30, 14)
template <class RNG>
...
void WL1dEvecGenerator<RNG>::writeState(const char *name) {
  ...
  if (ofile) {
    ...
    ofile << "],\n";
    // YingWai: should be written out only when potential shift != 0   (Sep 2,
    // 14)
    ofile << "\"potentialShifts\" : [\n";
    for (int j = 0; j < n_spins; j++)
      ofile << potentialShifts_pointer[j]
            << ((j == n_spins - 1) ? "\n" : ",\n");
    ofile << "],\n";
    ...
    for (int i = 0; i < n_walkers; i++)
      ofile << lastChangePotentialShift[i]
            << ((i == n_walkers - 1) ? "\n" : ",\n");
    ofile << "],\n";
...
// YingWai: Should deal with energy in the same bin the same way as in
// determineAcceptance
// ... later     (Feb 13, 14)
template <class RNG>
...
bool WL1dEvecGenerator<RNG>::determineAcceptance(int instance, double energy,
  ...
  if (ref1[instance] < 0 ||
      ref1[instance] > dos.getN() - 1)  // energy out of range, reject the move
  {
    out[instance] = true;
    accept_step = false;
  ...
  if (verbosity > 2) std::cout.precision(10);
  std::cout << "WangLandau 1d EvecGenerator step " << modificationFactorChanges
            << " : " << numberOfUpdatesSinceLastBoost << " : "
            << stepsSinceLastHistogramUpdate << " nX = " << dos.getN() << " ["
            << dos.getMinX() << ", " << dos.getMaxX() << "] "
            << (accept_step ? "accepted" : "rejected") << " Energy = " << energy
...
bool WL1dEvecGenerator<RNG>::updateHistogram(int instance, double *evecs,
                                             ...
                                             bool accepted) {
  if (accepted) {
    lastAcceptedPotentialShiftIndex[instance] =
        lastChangePotentialShift[instance];
    lastAcceptedPotentialShift[instance] =
...
bool WL1dEvecGenerator<RNG>::updateHistogram(int instance, double *evecs,
                                             bool accepted) {
  if (lastAccepted[instance] == -2 &&
      out[instance])  // initial state outside energy range
    return false;
  ...
  if (stepsSinceLastHistogramUpdate >= flipPerUpdate) {
    ...
    if (!histogramUpdateMode)  // standard Wang-Landau
    {
      ...
      dos.addMomentsAtIdx(dos.idx_withinRange(position[instance]),
...
template <class RNG>
bool WL1dEvecGenerator<RNG>::updateHistogramFromRE(int instance, double *evecs,
                                                   double energy,
                                                   double *potentialShifts,
                                                   int check) {
  return updateHistogramFromRE(instance, evecs, energy, 0.0, potentialShifts,
                               check);
}
...
bool WL1dEvecGenerator<RNG>::updateHistogramFromRE(int instance, double *evecs,
                                                   ...
                                                   double magnetization,
                                                   int check) {
  //  if (lastAccepted[instance] == -2 && out[instance])      // initial state
  //  outside energy range
  //    return false;
