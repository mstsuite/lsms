...
static void writeSingleEvec(FILE *f, int z, int i, Real posX, Real posY,
                            Real posZ, AtomData &atom) {
  // Z global_id x y z  qtotws  mtotws  evec_x evec_y evec_z  e_mix  B_x B_y B_z
  // vSpinShift
  fprintf(f,
          "%3d %8d  %21.15lf  %21.15lf  %21.15lf  %12.6lf %12.6lf  %21.15lf  "
          "%21.15lf  %21.15lf  %6.2lf  %21.15lf  %21.15lf  %21.15lf  %8.4lf  "
          "%21.15lf  %21.15lf  %21.15lf\n",
          z, i, posX, posY, posZ, atom.qtotws, atom.mtotws, atom.evec[0],
          atom.evec[1], atom.evec[2], -1.0, atom.b_con[0], atom.b_con[1],
          atom.b_con[2], atom.vSpinShift, atom.evecOut[0], atom.evecOut[1],
          atom.evecOut[2]);
}

// write out the magnetism and constraint info for each site
static void writeSingleLocalAtomData(FILE *f, int z, int i, Real posX,
                                     Real posY, Real posZ, AtomData &atom) {
  // Z global_id x y z  qtotws  mtotws  evec_x evec_y evec_z  B_x B_y B_z
  // vSpinShift localVolume localEnergy  mtotmt  mvalmt  mvalws  rmt jws
  fprintf(f,
          "%3d %8d  %21.15lf  %21.15lf  %21.15lf  %12.6lf %12.6lf  %21.15lf  "
...
static void readSingleEvec(FILE *f, int &z, int &i, Real &posX, Real &posY,
  ...
  auto retval = fscanf(
      ...
      &z, &i, &posX, &posY, &posZ, &tmp1, &atom.mtotws, &atom.evec[0],
      &atom.evec[1], &atom.evec[2], &tmp2, &atom.b_con[0], &atom.b_con[1],
      &atom.b_con[2], &atom.vSpinShift, &atom.evecOut[0], &atom.evecOut[1],
      &atom.evecOut[2]);
}

static void readSingleEvec(FILE *f, int &z, int &i, Real *pos, Real &mtotws,
                           Real *evec, Real *b_con, Real &vSpinShift,
                           Real *evecOut) {
  Real tmp1, tmp2;
  // Z global_id x y z  qtotws  mtotws  evec_x evec_y evec_z  e_mix  B_x B_y B_z
  // vSpinShift
  auto retval = fscanf(
      f,
      "%d %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n",
      &z, &i, &pos[0], &pos[1], &pos[2], &tmp1, &mtotws, &evec[0], &evec[1],
      &evec[2], &tmp2, &b_con[0], &b_con[1], &b_con[2], &vSpinShift,
  ...
  if (comm.rank == 0) {
    ...
    for (int i = 0; i < crystal.num_types; i++) {
      if (crystal.types[i].node == comm.rank) {
        ...
        readSingleEvec(inf, Z, ii, pos, mtotws, evec, b_con, spin_shift,
                       evec_out);
...
int writeLocalAtomData(LSMSCommunication &comm, LSMSSystemParameters &lsms,
  ...
  if (comm.rank == 0) {
    ...
    fprintf(outf,
            "# totalEnergy  bandEnergy  fermiEnergy electrostaticEnergy\n");
    fprintf(outf,
