...
int writeRestart(const char *restartName, LSMSSystemParameters &lsms,
  ...
  if (lsms.pot_out_type < 0)
    return 1;  // if we didn't write output files we probably don't want to
               // write a restart either
  ...
  for (int i = 1; i < numFunctionalIndices; i++) {
    if (lsms.xcFunctional[i] >= 0) fprintf(of, ",%d", lsms.xcFunctional[i]);
  }
  fprintf(of, "}\n\n");
  ...
  for (int i = 0; i < 3; i++) {
    fprintf(of, "bravais[%d]={%21.15lf, %21.15lf, %21.15lf}\n", i + 1,
            crystal.bravais(0, i), crystal.bravais(1, i),
            crystal.bravais(2, i));
  }
  ...
  for (int i = 0; i < crystal.num_atoms; i++) {
    ...
    fprintf(of, "site[%d].pos={%21.15lf, %21.15lf, %21.15lf}\n", i + 1,
            crystal.position(0, i), crystal.position(1, i),
            crystal.position(2, i));
    ...
    fprintf(of, "site[%d].atom=\"%s\"\n", i + 1,
            crystal.types[crystal.type[i]].name);
    fprintf(of, "site[%d].lmax=%d\n", i + 1,
            crystal.types[crystal.type[i]].lmax);
    ...
    if (crystal.types[crystal.type[i]].forceZeroMoment != 0)
      fprintf(of, "site[%d].forceZeroMoment=%d\n", i + 1,
              crystal.types[crystal.type[i]].forceZeroMoment);
    fprintf(of, "site[%d].rLIZ=%lf\n", i + 1,
            crystal.types[crystal.type[i]].rLIZ);
    fprintf(of, "site[%d].rsteps={%lf,%lf,%lf,%lf}\n", i + 1,
            crystal.types[crystal.type[i]].rsteps[0],
            crystal.types[crystal.type[i]].rsteps[1],
            crystal.types[crystal.type[i]].rsteps[2],
            crystal.types[crystal.type[i]].rsteps[3]);
    fprintf(of, "site[%d].rad=%lf\n", i + 1,
            crystal.types[crystal.type[i]].rad);

    // if(!luaGetIntegerFieldFromStack(L,"type",&t) || (t-1)==i)

    // luaGetIntegerFieldFromStack(L,"alloy_class",&crystal.types[crystal.num_types].alloy_class);
    // crystal.types[crystal.num_types].alloy_class--; // <-- zero-based
    // indexing
  }
