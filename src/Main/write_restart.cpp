                 ...
                 PotentialShifter &potentialShifter, AlloyMixingDesc& alloyDesc)
{
  ...
  if(lsms.pot_out_type < 0) return; // if we didn't write output files we probably don't want to write a restart either

  of = fopen(restartName,"w");
  ...
  for(int idx=0; idx<mix.numQuantities; idx++)
  {
    if(mix.quantity[idx])
    {
      ...
      if(idx == MixingParameter::charge)
      {
        fprintf(of, "\{quantity = \"charge\", ");
      } else if(idx == MixingParameter::potential) {
        fprintf(of, "\{quantity = \"potential\", ");
      } else if(idx == MixingParameter::moment_magnitude) {
         fprintf(of, "\{quantity = \"moment_magnitude\", ");
      } else if(idx == MixingParameter::moment_direction) {
        fprintf(of, "\{quantity = \"moment_direction\", ");
      } else if(idx == MixingParameter::no_mixing) {
  ...
  for(int i=0; i<crystal.num_atoms; i++)
  {
    ...
    fprintf(of,"site[%d].atom=\"%s\"\n",i+1,crystal.types[crystal.type[i]].name,4);
