...
static void repeatBasisCell(LSMSSystemParameters &lsms,
                            CrystalParameters &crystal, int nx, int ny, int nz,
                            int unique) {
  ...
  if (unique) {
    ...
    for (int i = 0; i < numBasis; i++)
      if (crystal.types[i].pot_in_idx < 0) crystal.types[i].pot_in_idx = i;
  }
    ...
    for (int iy = 0; iy < ny; iy++)
      for (int iz = 0; iz < nz; iz++) {
        for (int ib = 0; ib < numBasis; ib++) {
          crystal.position(0, i) = basis(0, ib) + ix * crystal.bravais(0, 0) +
              iy * crystal.bravais(0, 1) +
              iz * crystal.bravais(0, 2);
          crystal.position(1, i) = basis(1, ib) + ix * crystal.bravais(1, 0) +
              iy * crystal.bravais(1, 1) +
              iz * crystal.bravais(1, 2);
          crystal.position(2, i) = basis(2, ib) + ix * crystal.bravais(2, 0) +
              iy * crystal.bravais(2, 1) +
              iz * crystal.bravais(2, 2);
          if (unique) {
            crystal.type[i] = i;
            if (i >= numBasis) {
...
int readInput(lua_State *L, LSMSSystemParameters &lsms,
              CrystalParameters &crystal, lsms::MixingParameterPack &mix,
  ...
  char ctmp[32];
  strncpy(ctmp, "main", 32);
  luaGetStrN(L, "istop", ctmp, 32);
  lsms.global.setIstop(ctmp);
  ...
  char rel_str[80];
  rel_str[0] = 's';
  rel_str[1] = 0;  // default is scalar relativistic
  luaGetStrN(L, "relativity", rel_str, 40);
  switch (rel_str[0]) {
    case 'n':
    case 'N':lsms.relativity = none;
      lsms.nrelv = 10;
      lsms.nrelc = 10;
      break;
    case 's':
    case 'S':lsms.relativity = scalar;
      lsms.nrelv = 0;
      lsms.nrelc = 0;
      break;
    case 'f':
    case 'F':lsms.relativity = full;
      lsms.nrelv = 0;
      lsms.nrelc = 0;
      break;
  }
  ...
  */
  ...
  lsms.constraint = 0;
  luaGetInteger(L, "constraint", &lsms.constraint);
  ...
  for (int i = 0; i < numFunctionalIndices; i++) {
    luaGetIntegerPositionInTable(L, "xcFunctional", i + 1,
                                 &lsms.xcFunctional[i]);
  }
  ...
  for (int i = 0; i < 3; i++) {
    ...
    for (int j = 0; j < 3; j++)
      luaGetRealPositionFromStack(L, j + 1, &crystal.bravais(j, i));
    lua_pop(L, 2);
  }
  ...
  for (int i = 0; i < crystal.num_atoms; i++) {
    ...
    for (int j = 0; j < 3; j++)
      luaGetRealPositionFromStack(L, j + 1, &crystal.position(j, i));
    lua_pop(L, 1);
    ...
    if (!luaGetIntegerFieldFromStack(L, "type", &t) || (t - 1) == i) {
      ...
      luaGetIntegerFieldFromStack(L, "pot_in_idx",
                                  &crystal.types[crystal.num_types].pot_in_idx);
      luaGetIntegerFieldFromStack(L, "lmax",
                                  &crystal.types[crystal.num_types].lmax);
      luaGetIntegerFieldFromStack(L, "Z", &crystal.types[crystal.num_types].Z);
      luaGetIntegerFieldFromStack(L, "Zc",
                                  &crystal.types[crystal.num_types].Zc);
      luaGetIntegerFieldFromStack(L, "Zs",
                                  &crystal.types[crystal.num_types].Zs);
      luaGetIntegerFieldFromStack(L, "Zv",
                                  &crystal.types[crystal.num_types].Zv);
      crystal.types[crystal.num_types].forceZeroMoment = 0;
      luaGetIntegerFieldFromStack(
          L, "forceZeroMoment",
          &crystal.types[crystal.num_types].forceZeroMoment);
      luaGetIntegerFieldFromStack(
          L, "alloy_class", &crystal.types[crystal.num_types].alloy_class);
      luaGetIntegerFieldFromStack(
          L, "lsf", &crystal.types[crystal.num_types].lsf_functional);
      crystal.types[crystal.num_types]
          .alloy_class--;  // <-- zero-based indexing
      luaGetRealFieldFromStack(L, "rLIZ",
                               &crystal.types[crystal.num_types].rLIZ);
      ...
      for (int j = 0; j < 4; j++)
        luaGetRealPositionFromStack(
            L, j + 1, &crystal.types[crystal.num_types].rsteps[j]);
      lua_pop(L, 1);
    ...
    for (int j = 0; j < 3; j++)
      luaGetRealPositionFromStack(L, j + 1, &crystal.evecs(j, i));
    lua_pop(L, 1);
    lua_pop(L, 2);
  }
  ...
  for(int i = 0; i < alloyDesc.size(); i++) {
    std::cout << "class[" << i << "].ncomp = " << alloyDesc[i].size() <<
  std::endl; for(int j = 0; j < alloyDesc[i].size(); j++) { std::cout << "atom =
  " << alloyDesc[i][j].name << std::endl; std::cout << "Z  = " <<
  alloyDesc[i][j].Z << std::endl; std::cout << "Zc = " << alloyDesc[i][j].Zc <<
  std::endl; std::cout << "Zs = " << alloyDesc[i][j].Zs << std::endl; std::cout
  << "Zv = " << alloyDesc[i][j].Zv << std::endl;
    }
  }
  ...
  luaGetIntegerFieldInTable(L, "energyContour", "grid",
  ...
  luaGetRealFieldInTable(L, "energyContour", "eibot",
                         &lsms.energyContour.eibot);
  luaGetRealFieldInTable(L, "energyContour", "eitop",
                         &lsms.energyContour.eitop);
  luaGetIntegerFieldInTable(L, "energyContour", "npts",
                            &lsms.energyContour.npts);
  lsms.energyContour.maxGroupSize = 50;
  luaGetIntegerFieldInTable(L, "energyContour", "maxGroupSize",
  ...
  lsms.lsmsMode = LSMSMode::main;
  {
    ...
    if (luaGetStrN(L, "lsmsMode", h, 80)) {
      ...
      if (strncmp(h, "main", 80) == 0)
        lsms.lsmsMode = LSMSMode::main;
      else if (strncmp(h, "dos", 80) == 0)
        lsms.lsmsMode = LSMSMode::dos;
      else if (strncmp(h, "liz0", 80) == 0)
        lsms.lsmsMode = LSMSMode::liz0;
      else if (strncmp(h, "gf_out", 80) == 0)
        lsms.lsmsMode = LSMSMode::gf_out;
      else if (strncmp(h, "matsubara", 80) == 0)
        lsms.lsmsMode = LSMSMode::matsubara;
      else if (strncmp(h, "kkrmat", 80) == 0)
        lsms.lsmsMode = LSMSMode::kkrmat;
      else {
  ...
  mix.n_init_spin_iterations = 0;
  luaGetInteger(L, "n_init_spin_iterations", &mix.n_init_spin_iterations);

  mix.n_init_iterations = 10;
  luaGetInteger(L, "n_init_iterations", &mix.n_init_iterations);
  ...
  for (auto index = 0; index < len; index++) {
    ...
    if (type == "potential") {
      mix.pot_mixer_type = algo;
    } else if (type == "charge") {
      mix.chd_mixer_type = algo;
    } else if (type == "spin") {
      mix.spd_mixer_type = algo;
    } else if (type == "init") {
      mix.init_mixer_type = algo;
    } else if (type == "init_spin") {
      mix.init_spd_mixer_type = algo;
    } else {
    ...
    if (lua_type(L, -1) != LUA_TNIL) {
      ...
      if (type == "potential") {
        mix.potMixingParameter.alpha = alpha;
      } else if (type == "charge") {
        mix.chdMixingParameter.alpha = alpha;
      } else if (type == "spin") {
        mix.spdMixingParameter.alpha = alpha;
      } else if (type == "init") {
        mix.initMixingParameter.alpha = alpha;
      } else if (type == "init_spin") {
        mix.initSpdMixingParameter.alpha = alpha;
      } else {
        throw std::runtime_error("No implemented");
      }
    ...
    if (lua_type(L, -1) != LUA_TNIL) {
      ...
      if (type == "potential") {
        mix.potMixingParameter.w0 = w0;
      } else if (type == "charge") {
        mix.chdMixingParameter.w0 = w0;
      } else if (type == "spin") {
        mix.spdMixingParameter.w0 = w0;
      } else if (type == "init") {
        mix.initMixingParameter.w0 = w0;
      } else if (type == "init_spin") {
        mix.initSpdMixingParameter.w0 = w0;
      } else {
        throw std::runtime_error("No implemented");
      }
    };
    ...
    if (lua_type(L, -1) != LUA_TNIL) {
      ...
      if (type == "potential") {
        mix.potMixingParameter.max_broyden = max_broyden;
      } else if (type == "charge") {
        mix.chdMixingParameter.max_broyden = max_broyden;
      } else if (type == "spin") {
        mix.spdMixingParameter.max_broyden = max_broyden;
      } else if (type == "init") {
        mix.initMixingParameter.max_broyden = max_broyden;
      } else if (type == "init_spin") {
        mix.initSpdMixingParameter.max_broyden = max_broyden;
      } else {
        throw std::runtime_error("No implemented");
      }
    }
    ...
    if (lua_type(L, -1) != LUA_TNIL) {
      ...
      if (type == "potential") {
        mix.potMixingParameter.iter_reset = iter_reset;
      } else if (type == "charge") {
        mix.chdMixingParameter.iter_reset = iter_reset;
      } else if (type == "spin") {
        mix.spdMixingParameter.iter_reset = iter_reset;
      } else if (type == "init") {
        mix.initMixingParameter.iter_reset = iter_reset;
      } else if (type == "init_spin") {
        mix.initMixingParameter.iter_reset = iter_reset;
      } else {
        throw std::runtime_error("No implemented");
  ...
  if (lua_istable(L, -1)) {
    lua_pop(L, 1);
    luaGetIntegerFieldInTable(L, "potentialShift", "switch",
                              &potentialShiftSwitch);
    if (potentialShiftSwitch) {
      potentialShifter.vSpinShiftFlag = true;
      luaGetRealFieldInTable(L, "potentialShift", "shift_min",
                             &potentialShifter.minShift);
      luaGetRealFieldInTable(L, "potentialShift", "shift_max",
                             &potentialShifter.maxShift);
    }
  } else {
    lua_pop(L, 1);
  }
