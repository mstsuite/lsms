...
class EvecGenerator {
 public:
  virtual double getDOSRatio(int instance, double energy) { return 0.0; }

  // YingWai: do we really need the function overloading for
  // determineAcceptance?   (Dec 4, 13)
  ...
  */
  virtual bool updateHistogram(int instance, double *evecs,
                               double *potentialShifts, bool accepted) {
    return updateHistogram(instance, evecs, accepted);
  }

  virtual bool updateHistogramFromRE(int instance, double *evecs, double energy,
                                     int check) {
    return false;
  }
  ...
  /*
  // YingWai: keep these function overloadings for updateHistogram for the
  moment,
  //          but might want to get rid of them later when code is stable  (Dec
  4, 13)
  ...
  */
  ...
  virtual void generatePotentialShift(int instance, double *potentialShifts,
                                      bool accepted){};

  virtual void generateEvec(int instance, double *evecs, bool accepted){};

  virtual void initializeEvecAndPotentialShift(int instance, double *evecs,
  ...
  virtual void generateUnsampledEvec(int instance, double *evecs,
                                     double energy) {
  ...
  // these routines were added to the Evec class because they share Monte Carlo
  // logic
  virtual MoveChoice_t selectMoveType(bool isSpinSim, bool isOccSim) {
    return SpinMove;
  }
  ...
  virtual void updateLog(int instance, double *evecs, int *occ, double energy,
...
class ConstantEvecGenerator : public EvecGenerator {
 public:
  ...
  void generatePotentialShift(int instance, double *potentialShifts,
                              bool accepted) {
    for (size_t i = 0; i < n_spins; i++) potentialShifts[i] = 0.0;
  }
...
class RandomEvecGenerator : public EvecGenerator {
 public:
  ...
  void generateEvec(int inst, double *evecs, double energy) {
    for (size_t j = 0; j < 3 * n_spins; j += 3) random_evec(&evecs[j]);
    // return false;
  }
