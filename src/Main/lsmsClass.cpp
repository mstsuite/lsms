...
void setupVorpol(LSMSSystemParameters &lsms, CrystalParameters &crystal,
                 LocalTypeInfo &local, SphericalHarmonicsCoeficients &shc);
...
// PotentialShifter &ps) LSMS::LSMS(MPI_Comm _comm, const char* i_lsms, const
// char* out_prefix)
LSMS::LSMS(MPI_Comm _comm, const char *i_lsms, const char *out_prefix,
           int my_group) {
  myWalkerID = my_group;
  ...
  /*
    if (rank == 0)
      std::cout<<"initializing lsms::lsms with i_lsms="<<i_lsms<<" and
    out_prefix="
                 <<prefix<<std::endl;
  */
  ...
  if (alloyDesc.size() > 0) loadAlloyBank(comm, lsms, alloyDesc, alloyBank);
  ...
  if (lsms.global.iprint >= 0) printLocalTypeInfo(stdout, local);
  ...
  if (lsms.global.iprint >= 0) printf("Finished calculateCoreStates(...)\n");
  ...
  LSMS_version = 3000;
  //  if (comm.rank == 0)
  //    std::cout<<prefix<<" lsms::lsms - LSMS version is
  //    "<<LSMS_version<<std::endl;
  ...
  if (comm.rank == 0) {
    ...
    for (int p = 0; p < crystal.num_types; p++) {
      ...
      if (n == 0) {
        ...
        MPI_Isend(&(ev[p])[0], 3, MPI_DOUBLE, n, l, comm.comm,
                  &request[n_req++]);
    }
    for (int i = 0; i < n_req; i++) MPI_Wait(&request[i], &status);
  } else {
    std::vector<MPI_Request> request(local.num_local);
    for (int p = 0; p < local.num_local; p++)
      MPI_Irecv(&local.atom[p].evec[0], 3, MPI_DOUBLE, 0, p, comm.comm,
                &request[p]);
    for (int i = 0; i < local.num_local; i++) MPI_Wait(&request[i], &status);
  }
  ...
  if (comm.rank == 0) {
    ...
    for (int p = 0; p < crystal.num_types; p++) {
      ...
      if (n == 0) {
        ...
        MPI_Isend(&ev[3 * p], 3, MPI_DOUBLE, n, l, comm.comm,
                  &request[n_req++]);
    }
    for (int i = 0; i < n_req; i++) MPI_Wait(&request[i], &status);
  } else {
  ...
  if (comm.rank == 0) {
    ...
    for (int p = 0; p < crystal.num_types; p++) {
      ...
      if (n == 0) {
        ...
        MPI_Isend(&evAndSpinShifts[4 * p], 4, MPI_DOUBLE, n, l, comm.comm,
                  &request[n_req++]);
    }
    for (int i = 0; i < n_req; i++) MPI_Wait(&request[i], &status);
  } else {
    ...
    for (int p = 0; p < local.num_local; p++)
      MPI_Irecv(&recvBuffer[4 * p], 4, MPI_DOUBLE, 0, p, comm.comm,
                &request[p]);
...
void LSMS::getEvec(std::vector<std::vector<Real> > &ev) {
  ...
  for (int i = 0; i < max_num_local; i++) s_buf(0, i) = -1.0;
  ...
  MPI_Gather(&s_buf(0, 0), 4 * max_num_local, MPI_DOUBLE, &r_buf(0, 0, 0),
             4 * max_num_local, MPI_DOUBLE, 0, comm.comm);
...
void LSMS::getMag(std::vector<std::vector<Real> > &ev) {
  ...
  for (int i = 0; i < local.num_local; i++) {
    ...
    /*
        s_buf(1,i)=local.atom[i].dosckint[1] + local.atom[i].evec[0] *
       local.atom[i].mcpsc_mt; s_buf(2,i)=local.atom[i].dosckint[2] +
       local.atom[i].evec[1] * local.atom[i].mcpsc_mt;
        s_buf(3,i)=local.atom[i].dosckint[3] + local.atom[i].evec[2] *
...
void LSMS::getMag(Real *ev) {
  ...
  for (int i = 0; i < max_num_local; i++) s_buf[4 * i] = -1.0;
  ...
  MPI_Gather(s_buf, 4 * max_num_local, MPI_DOUBLE, r_buf, 4 * max_num_local,
             MPI_DOUBLE, 0, comm.comm);
  ...
  if (comm.rank == 0) {
    ...
    for (int p = 0; p < crystal.num_types; p++) {
      ...
      // if not owned by root, send site occupancy
        ...
        MPI_Isend(&occ[p], 1, MPI_INTEGER, owner, l, comm.comm,
                  &request[n_req++]);
      }
    }
    ...
    /* dprintf("Walker %d, LSMS %d: Recieved local occupancies:
    ",myWalkerID,comm.rank); for(int p = 0; p < local.num_local; p++)
      dprintf("%d",xi[p]);
    dprintf("\n"); */
    ...
    for (int p = 0; p < local.num_local; p++) {
      ...
      // dprintf("Walker %d, LSMS %d: Local Atom %d: Alloy Class
...
// oneStepEnergy calculates the frozen potential energy without converging the
// Fermi energy
...
Real LSMS::multiStepEnergy() {
  ...
  // calculate the Zeeman contribution from the spin shift and adjust the band
  // energy acordingly
  if (1 == 0)
    if (potentialShifter.vSpinShiftFlag) {
      calculateEvec(lsms, local);
      ...
      for (int i = 0; i < local.num_local; i++) {
        ...
        printf(" local.atom[%d].xvalws = %lf  %lf\n", i,
               local.atom[i].xvalwsNew[0], local.atom[i].xvalwsNew[1]);
        printf(" local.atom[%d].evec = %lf  %lf  %lf\n", i,
               local.atom[i].evec[0], local.atom[i].evec[1],
               local.atom[i].evec[2]);
        printf(" local.atom[%d].evecNew = %lf  %lf  %lf\n", i,
               local.atom[i].evecNew[0], local.atom[i].evecNew[1],
  ...
  if (lsms.global.iprint >= 0) {
    if (iterationCount < lsms.nscf)
      printf("LSMS::multiStepEnergy() converged in %d steps.\n",
             iterationCount);
    else
      printf("LSMS::multiStepEnergy() did not converge in %d steps.\n",
             lsms.nscf);
  }
  return eband;
}
...
Real LSMS::scfEnergy(Real *eb) {
  ...
  if (potentialShifter.vSpinShiftFlag) potentialShifter.applyShifts(local);
  ...
  for (iterationCount = 0; iterationCount < lsms.nscf && !converged;
       iterationCount++) {
    // if (lsms.global.iprint >= 0)
    ...
    // if(comm.rank==0) printf("Walker %d: On SCF iteration %d. Last RMS =
    ...
    if (1 == 0)
      if (potentialShifter.vSpinShiftFlag) {
        ...
        for (int i = 0; i < local.num_local; i++) {
          ...
          */
          eZeeman += 0.5 * s *
                     (local.atom[i].xvalwsNew[0] * local.atom[i].vSpinShift +
                      local.atom[i].xvalwsNew[1] * local.atom[i].vSpinShift);
        }
        globalSum(comm, eZeeman);
        if (lsms.global.iprint >= 0)
    ...
    for (int i = 0; i < local.num_local; i++) {
      if (local.atom[i].spinFlipped) {
        checkIfSpinHasFlipped(lsms, local.atom[i]);
        if (!local.atom[i].spinFlipped) swapCoreStateEnergies(local.atom[i]);
      }
    }

    lsms::calculateCoreStates(comm, lsms, local);

    // If charge is mixed, recalculate the potential  (need a flag for this from
    // input)
    ...
    if (potentialShifter.vSpinShiftFlag) lsms.totalEnergy -= eZeeman;
    ...
    if (kFile != NULL)
      fprintf(kFile, "%3d %20.12lf %12.6lf   %12.6lf\n", iterationCount,
              lsms.totalEnergy, lsms.chempot, rms);

    if (potentialShifter.vSpinShiftFlag) potentialShifter.applyShifts(local);

    // calculate core states for new potential if we are performing scf
    // calculations
    lsms::calculateCoreStates(comm, lsms, local);
