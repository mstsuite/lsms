...
class WL2dEvecGenerator : public EvecGenerator {
 public:
  ...
  bool updateHistogram(int instance, double *evecs, double energy,
                       double magnetization, bool accepted);
  /*
    bool updateHistogram(int instance, double *evecs, double energy, double
    magnetization, bool *accepted); bool updateHistogram(int instance, double
    *evecs, double energy, double magnetization)
    {
      bool h;
...
template <class RNG>
WL2dEvecGenerator<RNG>::WL2dEvecGenerator(int num_spins, int num_instances,
                                          double **ev_p,
  ...
  */
  ...
  if (init_file_name != NULL && init_file_name[0] != 0) {
    ...
    if (json_root == NULL || json_root->type != JSON_OBJECT) {
      ...
      std::cerr << "In WL2dEvecGenerator(" << init_file_name
    ...
    for (json_t *it = json_root->child; it != NULL; it = it->next) {
      ...
      else if (label == "intervalEnergy")
        intervalEnergy = atof(it->child->text);
      else if (label == "intervalMagnetization")
        intervalMagnetization = atof(it->child->text);
      else if (label == "kernelWidthEnergy")
        kernelWidthEnergy = atof(it->child->text);
      else if (label == "kernelWidthMagnetization")
        kernelWidthMagnetization = atof(it->child->text);
      ...
      //*/
        ...
        if (ix != dos.getNx()) {
          std::cout << "ERROR #(dos) " << ix << " != nX " << dos.getNx()
                    << std::endl;
          exit(1);
        }
      } else if (label == "histo") {
        ...
        if (ix != histo.getNx()) {
          std::cout << "ERROR #(histo) " << ix << " != nX " << histo.getNx()
                    << std::endl;
          exit(1);
        }
      } else if (label == "evecs") {
    ...
    if (nX > 0) {
      ...
      dos.setXRange(xMin, xMax);
      histo.setXRange(xMin, xMax);
    } else {
      dos.setDeltaAndClear(intervalEnergy, intervalMagnetization);
      histo.setDeltaAndClear(intervalEnergy, intervalMagnetization);
    }
...
bool WL2dEvecGenerator<RNG>::determineAcceptance(int instance, double energy,
  ...
  // we have to postpone geting the y position until we have checked the
  // validity of gridX
  ...
  if (gridX < 0 || gridX > dos.getNx() - 1) {
    dos.extendToX(energy - dosKernel.getWidthX());
    histo.extendToX(energy - histoKernel.getWidthX());
    dos.extendToX(energy + dosKernel.getWidthX());
    histo.extendToX(energy + histoKernel.getWidthX());
...
template <class RNG>
bool WL2dEvecGenerator<RNG>::updateHistogram(int instance, double *evecs,
                                             bool accepted) {
  std::cerr << "Need energy and magnetization for updateHistogram in "
               "WL2dEvecGenerator.\n";
...
bool WL2dEvecGenerator<RNG>::updateHistogram(int instance, double *evecs,
  ...
  if (stepsSinceLastHistogramUpdate >= flipPerUpdate) {
    ...
    if (!out[instance]) {
      ...
      // addKernel(histo, histoKernel, positionX[instance],
      // positionY[instance]);
  ...
  if (cycleCount >= updateCycle) {
    ...
    std::cout <<"# accepence ratio =
    "<<double(accept)/double(accept+reject)<<"\n"; std::cout <<"# current
    flatness = "<<currentFlatnessCriterion<<"\n";
    */
    ...
    // actually sampled
    double hMin = histo.getMinValWithBorders(dosKernel.getWidthX(),
                                             dosKernel.getWidthY());
...
template <class RNG>
void WL2dEvecGenerator<RNG>::generateEvec(int instance, double *evecs,
                                          bool accepted) {
  if (accepted) {
