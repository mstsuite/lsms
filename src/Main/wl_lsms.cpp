...
int main(int argc, char *argv[]) {
  ...
  bool return_moments_flag =
      true;  // true -> return all magnetic moments from lsms run at each step.
  ...
  int energyIndex = 1;  // index for the return value to use for the MC step (0:
                        // total energy, 1: band energy)
  ...
  for (int i = 0; i < argc; i++) {
    ...
    if (!strcmp("-energy_calculation", argv[i]))
      strncpy(energy_calculation_name, argv[++i], 64);

    if (!strcmp("-config_space", argv[i]))
      strncpy(config_space_name, argv[++i], 64);
  }
...
#define NUM_PAPI_EVENTS 4
  ...
  int papi_events
      [NUM_PAPI_EVENTS];  // =
                          // {PAPI_TOT_INS,PAPI_TOT_CYC,PAPI_FP_OPS,PAPI_VEC_INS};
  char *papi_event_name[] = {"PAPI_TOT_INS", "PAPI_FP_OPS",
                             "RETIRED_SSE_OPERATIONS:DOUBLE_ADD_SUB_OPS:DOUBLE_"
                             "MUL_OPS:DOUBLE_DIV_OPS:OP_TYPE",
                             "RETIRED_SSE_OPERATIONS:SINGLE_ADD_SUB_OPS:SINGLE_"
                             "MUL_OPS:SINGLE_DIV_OPS:OP_TYPE"};
  ...
  for (int i = 0; i < NUM_PAPI_EVENTS; i++) {
    if (PAPI_event_name_to_code(papi_event_name[i], &papi_events[i]) !=
        PAPI_OK) {
      // printline("Error in obtaining PAPI event code for:
      // "+ttos(papi_event_name[i]),
  ...
  // printline("PAPI: "+ttos(hw_counters)+" counters
  // available",std::cout,parameters.myrankWorld);
  if (hw_counters > NUM_PAPI_EVENTS) hw_counters = NUM_PAPI_EVENTS;
...
#define SVN_REV "unknown"
  ...
  if (rank == 0) {
    ...
    switch (energyCalculationMode) {
      case OneStepEnergy:
        std::cout << "oneStepEnergy [frozen potential band energy]"
                  << std::endl;
        break;
      case MultiStepEnergy:
        std::cout << "multiStepEnergy [frozen potential band energy with "
                     "converged Fermi energy]"
                  << std::endl;
        break;
      case ScfEnergy:
    ...
    if (isOccupancySim && isSpinSim) {
      std::cout
          << " Error: Exploring both occupancy and spin configurational\n";
    ...
    std::cout << " Processor alignment (process allocation quantization) = "
              << align << std::endl;
    switch (evec_generation_mode) {
      case Constant:
        std::cout << " Constant moments direction along " << ev0[0] << " "
                  << ev0[1] << " " << ev0[2] << std::endl;
        break;
      case Random:
        std::cout << " Random distribution of moments (no Wang-Landau)"
                  << std::endl;
        break;
      case WangLandau_1d:
        std::cout << " Wang-Landau for one continuous variable (energy)"
                  << std::endl;
        //      return_moments_flag = true;
        //      generator_needs_moment = true;
        break;
      ...
      case WangLandau_2d:
        ...
        switch (second_dimension) {
          case MagneticMoment:
            std::cout << "magnitude of magnetization)";
            break;
          case MagneticMomentX:
            std::cout << "x component of magnetization)";
            break;
          case MagneticMomentY:
            std::cout << "y component of magnetization)";
            break;
          case MagneticMomentZ:
            std::cout << "z component of magnetization)";
            break;
        }
        std::cout << std::endl;
        //      return_moments_flag = true;
  ...
  /*  if (num_lsms == 1)
    {
      ...
      if (rank == 0)
      {
        std::cout << "executing LSMS(C++) for " << lsms_calc.numSpins() << "
    atoms\n"; std::cout << "  LSMS version = " << lsms_calc.version() <<
    std::endl;
      }

      if (energyCalculationMode == OneStepEnergy)
        std::cout << "one step Energy = " << lsms_calc.oneStepEnergy() <<
    std::endl; else if (energyCalculationMode == MultiStepEnergy) std::cout <<
    "multi-step Energy = " << lsms_calc.multiStepEnergy() << std::endl; else if
    (energyCalculationMode == ScfEnergy) std::cout << "self-consistent Energy =
    " << lsms_calc.scfEnergy() << std::endl; else
      {
        std:: cout << "ERROR: Unknown energy calculation mode for lsms_calc in
    wl-lsms main!\n"; MPI_Abort(MPI_COMM_WORLD, 5);
      }
    }
    else */
  {
    walkerSteps.resize(num_lsms + 1);
    for (int i = 0; i < num_lsms + 1; i++) walkerSteps[i] = 0;
    ...
    if (my_group >= 0) {
      ...
      if (rank == 0 && my_group == 0) {
        std::cout << prefix << "executing LSMS(C++) for "
                  << lsms_calc.numSpins() << " atoms\n";
        std::cout << prefix << "  LSMS version = " << lsms_calc.version()
                  << std::endl;
      }
      ...
      while (!finished) {
        if (rank == 0) {
          ...
          MPI_Recv(recv_buffer, 4 * size_lsms, MPI_DOUBLE, 0, MPI_ANY_TAG,
                   MPI_COMM_WORLD, &status);

          // use op code to distinguish whether occupancy or spin variables
          // recieved
          op = status.MPI_TAG;
          // dprintf("Walker %d: Recieved op code %d.\n", my_group, op);
          if (op == 51 || op == 61) {
            for (int i = 0; i < 4 * size_lsms; i++) evec[i] = recv_buffer[i];
          } else if (op == 52 || op == 62) {
        ...
        /* recognized opcodes:
           ...
           OneStepEnergy : calclulate frozen potential band energy in one step
           (don't converge Ef) use only if the Fermi energy will not change due
           to MC steps! The only method available in LSMS_1.9 MultiStepEnergy :
           calculate frozen potential band energy after converging Fermi energy
           This should be the new default method. If the Fermi energy doesn't
           change multiStepEnergy only performs one step and should be
           equivalent to oneStepEnergy The tolerance for Ef convergence can be
           set with LSMS::setEfTol(Real). The default tolerance is set in the
           LSMS::LSMS constructor (currently 1.0e-6). The maximum number of
           steps is read from the LSMS input file 'nscf' parameter. ScfEnergy :
           this will calculate the selfconsistent total energy. The maximum
           number of steps is read from the LSMS input file 'nscf' parameter.
        ...
        */
        ...
        if (op == 5) {
          ...
          if (energyCalculationMode == OneStepEnergy)
            ...
            std::cout << "ERROR: Unknown energy calculation mode for lsms_calc "
                         "in wl-lsms main!\n";
            MPI_Abort(MPI_COMM_WORLD, 5);
          }
          r_values[0] = energy;
          ...
          // store the type of move performed as an extra variable in return
          // values
          if (MoveChoice == SpinMove)
            r_values[2] = 0x00;
          ...
          if (return_moments_flag) lsms_calc.getMag(&r_values[R_VALUE_OFFSET]);
          if (rank == 0) {
            if (return_moments_flag)
              MPI_Send(r_values, R_VALUE_OFFSET + 3 * size_lsms, MPI_DOUBLE, 0,
                       1005, MPI_COMM_WORLD);
            else
              MPI_Send(r_values, R_VALUE_OFFSET, MPI_DOUBLE, 0, 1005,
                       MPI_COMM_WORLD);
          }
          ...
          r_values[0] = -1.0;
          if (lsms_calc.vSpinShiftFlag() == true) r_values[0] = 1.0;
          ...
          if (rank == 0) {
            ...
            for (int i = 0; i < nclasses; i++) ncomps[i] = alloyDesc[i].size();

            MPI_Send(ncomps, nclasses, MPI_INTEGER, 0, 1012, MPI_COMM_WORLD);
            for (int i = 0; i < nclasses; i++)
              MPI_Send(alloyDesc[i].data(), ncomps[i] * sizeof(AtomType),
                       MPI_BYTE, 0, 1012, MPI_COMM_WORLD);

            MPI_Send(site_alloyclass, size_lsms, MPI_INTEGER, 0, 1012,
                     MPI_COMM_WORLD);
          ...
          if (rank == 0)
            printf(
                "Wang-Landau walker %d performed %ld energy contour "
                "integrations.\n",
      ...
      /*
            ...
            {
              printf("Size specified for Wang-Landau and in LSMS input file
      ...
      */
      ...
      if (potentialShifter.vSpinShiftFlag) {
        ...
        for (int i = 0; i < num_lsms; i++) {
          ...
          for (int j = 0; j < size_lsms; j++) vSpinShifts[i][j] = 0.0;
          evecsAndSpinShifts[i] =
              (double *)malloc(sizeof(double) * 4 * size_lsms);
        }
      }
      ...
      switch (evec_generation_mode) {
        ...
        case Constant:
          ...
          break;
        case WangLandau_1d:
          generator = new WL1dEvecGenerator<std::mt19937>(
              size_lsms, num_lsms, evecs, potentialShifter, wl_inf, wl_outf,
              wl_stepf, occs);
          break;
        case ExhaustiveIsing:
          generator = new ExhaustiveIsing1dEvecGenerator(
              size_lsms, num_lsms, evecs, wl_inf, wl_outf);
          break;
        case WangLandau_2d:
          generator = new WL2dEvecGenerator<std::mt19937>(
              size_lsms, num_lsms, evecs, wl_inf, wl_outf, wl_stepf);
          break;
        default:
          std::cerr << "The code should never arrive here: UNKNOWN EVEC "
                       "GENERATION MODE\n";
          exit(1);
      }
      ...
      //  'alloyDesc[ac][i]' is atom description of ith component of mixing
      //  class 'ac'
      ...
      if (isOccupancySim) {
        ...
        int nclasses, *ncomps, *site_alloyclass;
        MPI_Recv(&nclasses, 1, MPI_INTEGER, lsms_rank0[0], 1012, MPI_COMM_WORLD,
                 &status);

        ncomps = (int *)malloc(sizeof(int) * nclasses);
        site_alloyclass = (int *)malloc(sizeof(int) * size_lsms);
        MPI_Recv(ncomps, nclasses, MPI_INTEGER, lsms_rank0[0], 1012,
                 MPI_COMM_WORLD, &status);
        ...
        MPI_Recv(site_alloyclass, size_lsms, MPI_INTEGER, lsms_rank0[0], 1012,
                 MPI_COMM_WORLD, &status);
      ...
      if (potentialShifter.vSpinShiftFlag)
        for (int i = 0; i < num_lsms; i++)
          generator->initializeEvecAndPotentialShift(i, evecs[i],
                                                     vSpinShifts[i]);
      else
        for (int i = 0; i < num_lsms; i++)
      ...
      // then we need to set both sets of variables before beginning
      // calculations
      if (isSpinSim && isOccupancySim)
        for (int i = 0; i < num_lsms; i++) {
          ...
          if (potentialShifter.vSpinShiftFlag) {
            ...
            MPI_Send(evecsAndSpinShifts[i], 4 * size_lsms, MPI_DOUBLE,
                     lsms_rank0[i], 61, MPI_COMM_WORLD);
          } else
            MPI_Send(evecs[i], 3 * size_lsms, MPI_DOUBLE, lsms_rank0[i], 61,
                     MPI_COMM_WORLD);

          // set occupancies
          for (int j = 0; j < size_lsms; j++)
            send_buffer[j] = double(occs[i][j]);
          MPI_Send(send_buffer, size_lsms, MPI_DOUBLE, lsms_rank0[i], 62,
                   MPI_COMM_WORLD);
        }
      ...
      if (total_init_steps > 0) {
        for (int i = 0; i < num_lsms; i++) {
          std::cout << "starting initial calculation in group " << i
                    << std::endl;
          ...
          if (MoveChoice == SpinMove) {
            ...
            if (potentialShifter.vSpinShiftFlag) {
              ...
              MPI_Send(evecsAndSpinShifts[i], 4 * size_lsms, MPI_DOUBLE,
                       lsms_rank0[i], 51, MPI_COMM_WORLD);
            } else
              MPI_Send(evecs[i], 3 * size_lsms, MPI_DOUBLE, lsms_rank0[i], 51,
                       MPI_COMM_WORLD);
          } else if (MoveChoice == OccupancyMove) {
            // dprintf("Master: Sending trial occupancies to Walker %d.\n", i);
            for (int j = 0; j < size_lsms; j++)
              send_buffer[j] = double(occs[i][j]);
            MPI_Send(send_buffer, size_lsms, MPI_DOUBLE, lsms_rank0[i], 52,
                     MPI_COMM_WORLD);
          }
          ...
          walkerSteps[i + 1]++;
          if (restrict_steps)
            std::cout << "      " << num_steps << " steps remaining\n";
        }
        // first deal with the initial steps:
        while (running > 0) {
          if (return_moments_flag)
            MPI_Recv(r_values, R_VALUE_OFFSET + 3 * size_lsms, MPI_DOUBLE,
                     MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
          else
            MPI_Recv(r_values, R_VALUE_OFFSET, MPI_DOUBLE, MPI_ANY_SOURCE,
                     MPI_ANY_TAG, MPI_COMM_WORLD, &status);
          running--;

          // std::cout << "received energy E_tot =" << r_values[0] << std::endl;
          // std::cout << "    band energy E_band =" << r_values[1] <<
          // std::endl;
          ...
          if (total_init_steps > 0) {
            ...
            std::cout << "starting additional calculation in group " << r_group
                      << std::endl;

            MoveChoice = generator->selectMoveType(isSpinSim, isOccupancySim);
            if (init_steps[r_group] > 0) {
              if (MoveChoice == SpinMove)
                generator->generateUnsampledEvec(r_group, evecs[r_group],
                                                 r_values[energyIndex]);
              else if (MoveChoice == OccupancyMove)
                generator->generateUnsampledOcc(r_group, occs[r_group]);
              // more_work = !(generator -> generateUnsampledEvec(r_group,
              // evecs[r_group], r_values[energyIndex]));
              init_steps[r_group]--;
              total_init_steps--;
            }
            ...
            if (MoveChoice == SpinMove) {
              ...
              if (potentialShifter.vSpinShiftFlag) {
                for (int j = 0; j < size_lsms; j++) {
                  evecsAndSpinShifts[r_group][4 * j + 0] =
                      evecs[r_group][3 * j + 0];
                  evecsAndSpinShifts[r_group][4 * j + 1] =
                      evecs[r_group][3 * j + 1];
                  evecsAndSpinShifts[r_group][4 * j + 2] =
                      evecs[r_group][3 * j + 2];
                  evecsAndSpinShifts[r_group][4 * j + 3] =
                      vSpinShifts[r_group][j];
                }
                MPI_Send(evecsAndSpinShifts[r_group], 4 * size_lsms, MPI_DOUBLE,
                         lsms_rank0[r_group], 51, MPI_COMM_WORLD);
              } else
                MPI_Send(evecs[r_group], 3 * size_lsms, MPI_DOUBLE,
                         lsms_rank0[r_group], 51, MPI_COMM_WORLD);
            } else if (MoveChoice == OccupancyMove) {
              // dprintf("Master: Sending trial occupancies to Walker %d.\n",
              // r_group);
              ...
              MPI_Send(send_buffer, size_lsms, MPI_DOUBLE, lsms_rank0[r_group],
                       52, MPI_COMM_WORLD);
            }
      ...
      while (running > 0) {
        ...
        if (r_values[2] == 0x00) {
          ...
          // dprintf("Master: Recieved energy from Walker %d after spin
          // trial.\n",r_group);
        } else if (r_values[2] == 0x01) {
          prevMoveChoice = OccupancyMove;
          // dprintf("Master: Recieved energy from Walker %d after occupancy
          // trial.\n",r_group);
        }
        ...
        if (more_work) {
          ...
          if (generator_needs_moment) {
            ...
            switch (second_dimension) {
              case MagneticMoment:
                magnetization = std::sqrt(m0 * m0 + m1 * m1 + m2 * m2);
                break;
              case MagneticMomentX:
                magnetization = m0;
                break;
            ...
            // todo: separately keep track of whether last evec or occ move
            // accepted Determine if the configuration is accepted
            accepted = generator->determineAcceptance(
                r_group, r_values[energyIndex], magnetization);
          } else {
            // Determine if the configuration is accepted
            accepted =
                generator->determineAcceptance(r_group, r_values[energyIndex]);
          }
          ...
          // need to separate track of whether the last spin or occupancy change
          // was accepted this ensures the Wang-Landau generator can properly
          ...
          // dprintf("Master: Updating histogram after Walker %d
          // move.\n",r_group);
          if (potentialShifter.vSpinShiftFlag) {
            if (generator->updateHistogram(r_group, evecs[r_group],
                                           vSpinShifts[r_group], accepted))
              more_work = false;
          } else {
          ...
          if (MoveChoice == SpinMove) {
            // dprintf("Master: Generating trial spins for Walker
            // %d.\n",r_group);
            generator->generateEvec(r_group, evecs[r_group],
                                    acceptedSpinMove[r_group]);
            if (potentialShifter.vSpinShiftFlag)
              generator->generatePotentialShift(r_group, vSpinShifts[r_group],
                                                acceptedSpinMove[r_group]);
          } else if (MoveChoice == OccupancyMove) {
            // dprintf("Master: Generating trial occupancies for Walker
            // %d.\n",r_group);
            generator->generateOccupancies(r_group, occs[r_group],
                                           acceptedOccMove[r_group]);
          }
          ...
          if (MoveChoice == SpinMove) {
            ...
            // dprintf("Master: Sending trial occupancies for Walker
            // %d.\n",r_group);
            for (int j = 0; j < size_lsms; j++)
              send_buffer[j] = double(occs[r_group][j]);
            MPI_Send(send_buffer, size_lsms, MPI_DOUBLE, lsms_rank0[r_group],
          ...
          MPI_Send(evecs[r_group], 3 * size_lsms, MPI_DOUBLE,
                   lsms_rank0[r_group], 2, MPI_COMM_WORLD);
        }
        ...
        if (step_out_flag && accepted) {
          ...
          for (int j = 0; j < 3 * size_lsms; j += 3)
            step_out_file << r_values[j + R_VALUE_OFFSET] << "  "
                          << r_values[j + R_VALUE_OFFSET + 1] << "  "
      ...
      walkerSteps[0] =
          stepCount;  // report the total step count for the WL master
      std::ofstream statOut;
      statOut.open("energyLoopCount.statistics");
      statOut << rb[2 * 0] << "   " << walkerSteps[0] << "   " << rb[2 * 0 + 1]
              << std::endl;
      for (int i = 0; i < num_lsms; i++)
        statOut << rb[2 * lsms_rank0[i]] << "   " << walkerSteps[i + 1] << "   "
                << rb[2 * lsms_rank0[i] + 1] << std::endl;
      statOut.close();
      /*
        if(evec_generation_mode==WangLandau_1d)
        (static_cast<WL1dEvecGenerator<std::mt19937>
        *>(generator))->writeState("WLrestart.state");
        if(evec_generation_mode==ExhaustiveIsing)
        (static_cast<ExhaustiveIsing1dEvecGenerator
        *>(generator))->writeState("WLrestart.state");
      */
  ...
  if (world_rank == 0) {
    if (step_out_flag) {
      step_out_file << "# end\n-1\n"
                    << energy_accumulator / double(energies_accumulated)
                    << std::endl;
      step_out_file.close();
    }
    std::cout << "Finished all scheduled calculations. Freeing resources.\n";
    std::cout << "Energy mean = "
              << energy_accumulator / double(energies_accumulated) << "Ry\n";
  }
