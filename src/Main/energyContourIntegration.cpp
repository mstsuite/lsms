...
void green_function_(int *mtasa, int *n_spin_pola, int *n_spin_cant, int *lmax,
                     ...
                     Complex *zlr, Complex *jlr, int *nprpts, int *nplmax,
                     int *ngaussr, Real *cgnt, int *lmax_cg, Complex *dos,
                     Complex *dosck, Complex *green, Complex *dipole,
                     Complex *greenIntLLp, int *ncrit, Real *grwylm,
                     Real *gwwylm, Complex *wylm, int *iprint, char *istop,
                     int len_sitop);

void green_function_rel_(int *mtasa, int *lmax, int *kkrsz, Complex *wx,
                         Complex *wy, Complex *wz, Real *rins, Real *r_mesh,
                         int *jmt, int *jws, Real *h, Complex *pnrel,
                         Complex *tau00_l, Complex *matom, Complex *gz,
                         Complex *fz, Complex *gj, Complex *fj, int *nuz,
                         int *indz, int *nprpts, int *ngaussr, Real *cgnt,
                         int *lmax_cg, Complex *dos, Complex *dosck,
                         Complex *green, Complex *dipole, Complex *dos_orb,
                         Complex *dosck_orb, Complex *dens_orb, int *iprint,
...
void buildEnergyContour(int igrid, Real ebot, Real etop, Real eibot, Real eitop,
                        Real temperature, std::vector<Complex> &egrd,
                        std::vector<Complex> &dele1, int npts, int &nume,
  ...
  switch (igrid) {
    case 0:  // single energy point
      egrd.resize(1);
      dele1.resize(1);
      egrd[0] = std::complex<Real>(ebot, eibot);
      dele1[0] = 0.0;
      nume = 1;
      break;
  ...
  for (int i = 0; i < local.num_local; i++) {
    ...
    if (lsms.n_spin_cant == 2) {
      ...
      if (lsms.relativity != full) {
        ...
        int matrot_size =
            2 * (local.atom[i].lmax + 1) * (local.atom[i].lmax + 1);
  ...
  */
  ...
  if (lsms.lsmsMode == LSMSMode::dos) {
    ...
    if (lsms.energyContour.grid != 3 && comm.rank == 0)
      printf(
          "WARNING: lsms.energyContour.grid (%d) != 3 in dos calculation.\n   "
  ...
  buildEnergyContour(lsms.energyContour.grid, lsms.energyContour.ebot, e_top,
                     lsms.energyContour.eibot, lsms.energyContour.eitop,
                     lsms.temperature, egrd, dele1, lsms.energyContour.npts,
                     nume, lsms.global.iprint, lsms.global.istop);
  ...
  if (lsms.lsmsMode == LSMSMode::dos) {
    ...
    if (lsms.global.iprint > 0) {
      printf(
          "\nGenerate %lu energy points from %g Ry to %g Ry with imaginary "
          "part i%g Ry for DOS calculation",
          egrd.size(), egrd[0].real(), egrd[egrd.size() - 1].real(),
          egrd[0].imag());
    }
  ...
  std::vector<std::vector<NonRelativisticSingleScattererSolution> >
      solutionNonRel;
  std::vector<std::vector<RelativisticSingleScattererSolution> > solutionRel;

  if (lsms.relativity != full) {
    solutionNonRel.resize(lsms.energyContour.groupSize());
    for (int ie = 0; ie < lsms.energyContour.groupSize(); ie++)
      solutionNonRel[ie].resize(local.num_local);
  } else {
    solutionRel.resize(lsms.energyContour.groupSize());
    for (int ie = 0; ie < lsms.energyContour.groupSize(); ie++)
      solutionRel[ie].resize(local.num_local);
  }

  int maxkkrsz = (lsms.maxlmax + 1) * (lsms.maxlmax + 1);
  int maxkkrsz_ns = lsms.n_spin_cant * maxkkrsz;
  // for non spin canted, but spin polarized we store tau00_l for local site i
  // for spin up and down as tau00_l(*,i) and tau00_l(*,i + local.num_local)
  // i.e. tau00_l hase size (maxkkrsz_ns*maxkkrsz_ns,2*local.num_local)
  // n.b. n_spin_pola/n_spin_cant == 1 if non polarized or spin canted; == 2 iff
  // collinear
  ...
  // dipole=0.0;
  Array3d<Complex> green(
      local.maxjws(), 4,
      local.num_local);  // would be better as std::vector<Matrix<Complex>> to
                         // avoid problems with different jws.
  // Array3d<Complex> green(local.atom[0].jws,4,local.num_local); // would be
  // better as std::vector<Matrix<Complex>> to avoid problems with different
  // jws. or declare as lsms.global.iprpts instead!
  // green=0.0;
  ...
  deviceStorage->allocate(maxkkrsz, lsms.n_spin_cant, maxNumLIZ,
...
#pragma omp parallel for default(none) shared(local, dos, dosck, dipole, green)
  ...
  int numEGroups =
      nume / lsms.energyContour.groupSize() + std::min(1, eGroupRemainder);
  std::vector<int> eGroupIdx(numEGroups + 1);
  for (int ig = 0; ig < numEGroups; ig++)
    eGroupIdx[ig] = ig * lsms.energyContour.groupSize();
...
#pragma omp parallel for default(none) \
    shared(local, lsms, eGroupIdx, ig, egrd, solutionNonRel, vr_con)
      for (int ie = eGroupIdx[ig]; ie < eGroupIdx[ig + 1]; ie++) {
        int iie = ie - eGroupIdx[ig];
        Complex energy = egrd[ie];
        Complex pnrel = std::sqrt(energy);

        solveSingleScatterers(lsms, local, vr_con, energy, solutionNonRel[iie],
                              iie);
      }
    } else {
#pragma omp parallel for default(none) \
    shared(local, lsms, eGroupIdx, ig, egrd, solutionRel, vr_con)
      for (int ie = eGroupIdx[ig]; ie < eGroupIdx[ig + 1]; ie++) {
        int iie = ie - eGroupIdx[ig];
        Complex energy = egrd[ie];

        solveSingleScatterers(lsms, local, vr_con, energy, solutionRel[iie],
                              iie);
      }
    }
    ...
    if (lsms.global.iprint >= 2)
      printf("About to finalize t matrices communication\n");
...
#endif
    if (lsms.global.iprint >= 0)
      printf("timeSingleScatteres = %lf sec\n", timeSingleScatterers);
    ...
    if (buildKKRMatrixKernel == MST_BUILD_KKR_MATRIX_ACCELERATOR) {
      ...
      deviceStorage->copyTmatStoreToDevice(local.tmatStore,
...
// firstprivate(ie,iie,pnrel,energy,nume)
          for(int i=0; i<local.num_local; i++)
          {
            ...
            Array3d<Complex> greenIntLLp(local.atom[i].kkrsz,
                                         local.atom[i].kkrsz,
                                         lsms.n_spin_cant * lsms.n_spin_cant);
            green_function_(
                &lsms.mtasa, &lsms.n_spin_pola, &lsms.n_spin_cant,
                &local.atom[i].lmax, &local.atom[i].kkrsz, &local.atom[i].wx[0],
                &local.atom[i].wy[0], &local.atom[i].wz[0], &rins, &r_sph,
                &local.atom[i].r_mesh[0], &jmt, &local.atom[i].jws, &pnrel,
                &tau00_l(0, i), &solutionNonRel[iie][i].matom(0, 0),
                &solutionNonRel[iie][i].zlr(0, 0, 0),
                &solutionNonRel[iie][i].jlr(0, 0, 0), &nprpts, &nplmax,
                &lsms.ngaussr, &GauntCoeficients::cgnt(0, 0, 0),
                &GauntCoeficients::lmax, &dos(0, i), &dosck(0, i),
                &green(0, 0, i), &dipole(0, 0, i), &greenIntLLp(0, 0, 0),
                &local.atom[i].voronoi.ncrit,
                &local.atom[i].voronoi.grwylm(0, 0),
                &local.atom[i].voronoi.gwwylm(0, 0),
                &local.atom[i].voronoi.wylm(0, 0, 0), &lsms.global.iprint,
                lsms.global.istop, 32);
            if ((lsms.n_spin_pola == 2) &&
                (lsms.n_spin_cant == 1))  // spin polarized, collinear case
            {
              green_function_(
                  &lsms.mtasa, &lsms.n_spin_pola, &lsms.n_spin_cant,
                  &local.atom[i].lmax, &local.atom[i].kkrsz,
                  &local.atom[i].wx[0], &local.atom[i].wy[0],
                  &local.atom[i].wz[0], &rins, &r_sph, &local.atom[i].r_mesh[0],
                  &jmt, &local.atom[i].jws, &pnrel,
                  &tau00_l(0, i + local.num_local),
                  &solutionNonRel[iie][i].matom(0, 1),
                  &solutionNonRel[iie][i].zlr(0, 0, 1),
                  &solutionNonRel[iie][i].jlr(0, 0, 1), &nprpts, &nplmax,
                  &lsms.ngaussr, &GauntCoeficients::cgnt(0, 0, 0),
                  &GauntCoeficients::lmax, &dos(1, i), &dosck(1, i),
                  &green(0, 1, i), &dipole(0, 0, i), &greenIntLLp(0, 0, 0),
                  &local.atom[i].voronoi.ncrit,
                  &local.atom[i].voronoi.grwylm(0, 0),
                  &local.atom[i].voronoi.gwwylm(0, 0),
                  &local.atom[i].voronoi.wylm(0, 0, 0), &lsms.global.iprint,
            ...
            if (local.atom[i].forceZeroMoment && (lsms.n_spin_pola > 1)) {
              if (lsms.n_spin_cant > 1)  // spin canted case
              {
                for (int ir = 0; ir < green.l_dim1(); ir++) {
                  // green(ir,0,i) is the charge density part and
                  // green(ir,1:3,i) are the magnetic moment part
                  green(ir, 1, i) = green(ir, 2, i) = green(ir, 3, 0) = 0.0;
                }
                dos(1, i) = dos(2, i) = dos(3, i) = dosck(1, i) = dosck(2, i) =
                    dos(3, i) = 0.0;
            ...
            Complex tr_pxtau[3];
            calculateDensities(lsms, i, 0, ie, nume, energy, dele1[ie], dos,
                               dosck, green, dipole, local.atom[i]);
            if ((lsms.n_spin_pola == 2) &&
                (lsms.n_spin_cant == 1))  // spin polarized, collinear case
            {
              calculateDensities(lsms, i, 1, ie, nume, energy, dele1[ie], dos,
                                 dosck, green, dipole, local.atom[i]);
            }
          }
          ...
          for (int i = 0; i < local.num_local; i++) {
            ...
            /*
              ...
              {
              printf("gz(%d,%d,%d) = %f
              %f\n",j1,j2,j3,solutionRel[iie][i].gz(j1,j2,j3).real(),
              solutionRel[iie][i].gz(j1,j2,j3).imag()); printf("gj(%d,%d,%d) =
              %f %f\n",j1,j2,j3,solutionRel[iie][i].gj(j1,j2,j3).real(),
              solutionRel[iie][i].gj(j1,j2,j3).imag()); printf("fz(%d,%d,%d) =
              %f %f\n",j1,j2,j3,solutionRel[iie][i].fz(j1,j2,j3).real(),
              solutionRel[iie][i].fz(j1,j2,j3).imag()); printf("fj(%d,%d,%d) =
              %f %f\n",j1,j2,j3,solutionRel[iie][i].fj(j1,j2,j3).real(),
              solutionRel[iie][i].fj(j1,j2,j3).imag());
              }
            */

            green_function_rel_(
                &lsms.mtasa, &local.atom[i].lmax, &local.atom[i].kkrsz,
                &local.atom[i].wx[0], &local.atom[i].wy[0],
                &local.atom[i].wz[0], &rins, &local.atom[i].r_mesh[0],
                &local.atom[i].jws,  // originally jmt
                &local.atom[i].jws, &local.atom[i].h, &pnrel, &tau00_l(0, i),
                &solutionRel[iie][i].matom(0, 0),
                &solutionRel[iie][i].gz(0, 0, 0),
                &solutionRel[iie][i].fz(0, 0, 0),
                &solutionRel[iie][i].gj(0, 0, 0),
                &solutionRel[iie][i].fj(0, 0, 0), &solutionRel[iie][i].nuz[0],
                &solutionRel[iie][i].indz(0, 0), &nprpts, &lsms.ngaussr,
  ...
  if (lsms.lsmsMode == LSMSMode::dos) {
    ...
    if (lsms.n_spin_pola == 1)  // non spin polarized
    {
      ...
      for (int ia = 0; ia < local.num_local; ia++)
        for (int ie = 0; ie < nume; ie++) {
          dosOut[ie] +=
              local.atom[ia].dos_real(ie, 0) + local.atom[ia].dos_real(ie, 1);
          dosUpOut[ie] += local.atom[ia].dos_real(ie, 0);
          dosDownOut[ie] += local.atom[ia].dos_real(ie, 1);
          /*
          dosOut[ie] += local.atom[ia].dos_real(ie, 0); // +
          local.atom[ia].dos_real(ie,1); dosUpOut[ie] +=
          0.5*(local.atom[ia].dos_real(ie, 0) + local.atom[ia].dos_real(ie,1));
          dosDownOut[ie] +=  0.5*(local.atom[ia].dos_real(ie, 0) -
          local.atom[ia].dos_real(ie,1));
          */
        }
      ...
      for (int ia = 0; ia < local.num_local; ia++)
        for (int ie = 0; ie < nume; ie++) {
          ...
          Real t2 = std::sqrt(
              local.atom[ia].dos_real(ie, 1) * local.atom[ia].dos_real(ie, 1) +
              local.atom[ia].dos_real(ie, 2) * local.atom[ia].dos_real(ie, 2) +
              local.atom[ia].dos_real(ie, 3) * local.atom[ia].dos_real(ie, 3));
          dosOut[ie] += local.atom[ia].dos_real(
              ie, 0);  // + local.atom[ia].dos_real(ie,3);
