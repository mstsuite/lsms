...
*/
  ...
  Matrix<Complex> tau00_l(maxkkrsz_ns * maxkkrsz_ns, local.num_local * lsms.n_spin_pola /
                                                     lsms.n_spin_cant); // This would be cleaner as a std::vector<Matrix<Complex>>
  ...
  // dipole=0.0;
  Array3d<Complex> green(local.maxjws(), 4,
                         local.num_local); // would be better as std::vector<Matrix<Complex>> to avoid problems with different jws.
...
//        int nplmax=solutionNonRel[iie][i].zlr.l_dim2()-1;
            ...
            /*
             * Multipole moments: Only work for the non-spin polarized case at the moment
             */
...
//        int nplmax=solutionNonRel[iie][i].zlr.l_dim2()-1;
  ...
  /*
   * Display all integerated moments
   */
    ...
    for (int i = 0; i < local.num_local; i++) {
      ...
      if (local.atom[i].lmax_mom >= 0) {
        ...
        for (auto l = 0; l <= local.atom[i].lmax_mom; l++) {
          for (auto m = -l; m <= l; m++) {
            auto mm = local.atom[i].multipole_moms[k];
            std::printf("MM: %2d (%2d,%2d) %lf %lf\n", i, l, m, std::real(mm), std::imag(mm));
            k++;
          }
        }
