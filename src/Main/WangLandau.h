...
void inline performGlobalUpdate(Graph1dMoments<double, double> &g, double kappa,
...
class StatesWriter {
 public:
  ...
  void writeHeader(double lnF, int numWalk, int numSpin, double **spins,
                   bool isSpinSim = true, bool isOccSim = false,
                   int **occup = NULL) {
    if (!writeFlag) return;
  ...
  void writeChange(int iWalk, int numRet, int ispin, double *ev, double E) {
    if (writeFlag)
      of << iWalk << " " << numRet << " " << ispin << " " << ev[0] << " "
         << ev[1] << " " << ev[2] << " " << E << std::endl;
  }
  void writeChangeOcc(int iWalk, int numRet, int i, int j, int occ_i, int occ_j,
                      double E) {
    if (!writeFlag) return;
    of << iWalk << " " << numRet << " (" << i << "<-->" << j << ") " << occ_i
       << " " << occ_j << " " << E << std::endl;
  }
...
class WL1dEvecGenerator : public EvecGenerator {
 public:
  ...
  /*
    bool updateHistogram(int instance, double *evecs, double energy, bool
    *accepted); bool updateHistogram(int instance, double *evecs, double energy,
    double magnetization, bool *accepted); bool updateHistogram(int instance,
    double *evecs, double energy) {bool h; return updateHistogram(instance,
    evecs, energy, &h);} bool updateHistogram(int instance, double *evecs)
    {std::cerr<<"Need energy for WL1dEvecGenerator\n"; exit(1);}
  */

  void generateEvec(int instance, double *evecs, bool accepted);
  // void generateEvec(int instance, double *evecs, double energy);
  void generatePotentialShift(int instance, double *potentialShifts,
                              bool accepted);

  void initializeEvecAndPotentialShift(int inst, double *evecs,
  ...
  void startSampling(bool isSpinSim = true, bool isOccSim = false) {
    sw.writeHeader(gamma, n_walkers, n_spins, evecs_pointer, isSpinSim,
                   isOccSim, occupancy_ptr);
  }
  ...
  void updateLog(int instance, double *evecs, int *occ, double energy,
                 bool accepted, MoveChoice_t MoveChoice, bool isspin,
                 bool isocc);
...
#ifdef ISING
  void inline random_evec(double ev[3]) { ev[2] = -ev[2]; }
#else
...
template <class RNG>
WL1dEvecGenerator<RNG>::WL1dEvecGenerator(
    int num_spins, int num_instances, double **ev_p,
    PotentialShifter &potentialShifter, const char *init_file_name,
    const char *out_file_name, const char *states_filename, int **occ_p)
    : sw(states_filename) {
  ...
  for (int i = 0; i < n_walkers; i++) ref0[i] = -1;  // ref0[i]=HUGE;
  ...
  for (int i = 0; i < n_walkers; i++) lastSwapOcc[i].first = -1;
  ...
  */
  ...
  if (init_file_name != NULL && init_file_name[0] != 0) {
    ...
    std::cout << "Reading Wang-Landau configuration from: " << init_file_name
              << std::endl;
    ...
    std::string line;
    while (std::getline(inp, line)) buff << line << std::endl;
    ...
    if (json_root == NULL || json_root->type != JSON_OBJECT) {
      ...
      std::cerr << "In WL1dEvecGenerator(" << init_file_name
    ...
    for (json_t *it = json_root->child; it != NULL; it = it->next) {
      ...
      if (label == "xMin")
        ...
        interval = atof(it->child->text);
      else if (label == "kernelWidth")
        kernelWidth = atof(it->child->text);
      else if (label == "kernelType") {
        std::string strValue(it->child->text);
        kernelType = getKernelType(strValue);
      } else if (label == "gamma")
        gamma = atof(it->child->text);
      else if (label == "gammaFinal")
        gammaFinal = atof(it->child->text);
      else if (label == "nX") {
        ...
        */
      } else if (label == "flipPerUpdate")
        flipPerUpdate = atoi(it->child->text);
      else if (label == "updateCycle")
        updateCycle = atoi(it->child->text);
      else if (label == "cycleCount")
        cycleCount = atoi(it->child->text);
      else if (label == "changeMode")
        changeMode = atoi(it->child->text);
      else if (label == "flatnessCriterion")
        flatnessCriterion = atof(it->child->text);
      else if (label == "histogramMinimum")
        hMinimum = atof(it->child->text);
      else if (label == "updatesPerBin")
        updatesPerBin = atoi(it->child->text);
      else if (label == "globalUpdate.frequency")
        globalUpdate.frequency = atoi(it->child->text);
      else if (label == "globalUpdate.changes")
        globalUpdate.changes = atoi(it->child->text);
      else if (label == "globalUpdate.kappa")
        globalUpdate.kappa = atof(it->child->text);
      else if (label == "globalUpdate.lambda")
        globalUpdate.lambda = atof(it->child->text);
      else if (label == "globalUpdate.omega")
        globalUpdate.omega = atof(it->child->text);
      else if (label == "seed")
        rng.seed(atoi(it->child->text));
      else if (label == "accept")
        accept = atol(it->child->text);
      else if (label == "acceptSinceLastChange")
        acceptSinceLastChange = atol(it->child->text);
      else if (label == "reject")
        reject = atol(it->child->text);
      //*
      ...
      //*/
        ...
        if (j != dos.getN()) {
          std::cout << "ERROR #(dos) " << j << " != nX " << dos.getN()
                    << std::endl;
          exit(1);
        }
      } else if (label == "histo") {
        ...
        if (j != histo.getN()) {
          std::cout << "ERROR #(histo) " << j << " != nX " << histo.getN()
                    << std::endl;
          exit(1);
        }
      ...
      // mirrors 'evecs' read
        ...
        for (json_t *i = a->child; i != NULL; i = i->next) {
          if (n_initialized_from_file_occ < n_walkers) {
            ...
            if (k != n_spins) {
              std::cout << "ERROR #(occs) " << k << " != n_spins " << n_spins
                        << std::endl;
              exit(1);
            }
          }
        ...
        for (json_t *i = a->child; i != NULL; i = i->next) {
          if (n_initialized < n_walkers) {
            ...
            n_initialized++;
            if (k != 2) {
              std::cout << "ERROR #(lastSwapOcc) " << k << " != 2" << std::endl;
              exit(1);
            }
          }
        }
      ...
      //
        ...
        for (json_t *i = a->child; i != NULL; i = i->next) {
          if (n_initialized_from_file < n_walkers) {
            ...
            oldSpin[3 * n_initialized_from_file] =
                evecs_pointer[n_initialized_from_file][0];
            oldSpin[1 + 3 * n_initialized_from_file] =
                evecs_pointer[n_initialized_from_file][1];
            oldSpin[2 + 3 * n_initialized_from_file] =
                evecs_pointer[n_initialized_from_file][2];
            //
            n_initialized_from_file++;
            if (k != 3 * n_spins) {
              std::cout << "ERROR #(evecs) " << k << " != 3*n_spins "
                        << 3 * n_spins << std::endl;
              exit(1);
            }
          }
      ...
      } else if (label == "stepsSinceLastHistogramUpdate")
        stepsSinceLastHistogramUpdate = atoi(it->child->text);
      else if (label == "numberOfUpdatesSinceLastBoost")
        numberOfUpdatesSinceLastBoost = atoi(it->child->text);
      else if (label == "modificationFactorChanges")
        modificationFactorChanges = atoi(it->child->text);
      else if (label == "clearHistogram")
        clearHistogram = atoi(it->child->text);
      else if (label == "setFirstWalkerToFM")
        setFirstWalkerToFM = atoi(it->child->text);
      else if (label == "maskedHistogram")
        maskedHistogram = (atoi(it->child->text) != 0);
        ...
        std::cout << "WARNING: unknown label: " << label << std::endl;
    }
  ...
  if (setFirstWalkerToFM != 0) {
    ...
    std::cout << "Setting first " << std::min(setFirstWalkerToFM, n_walkers)
              << " walkers to FM.\n";
    // }
    n_initialized_from_file =
        std::max(setFirstWalkerToFM, n_initialized_from_file);
    for (int n = 0; n < std::min(setFirstWalkerToFM, n_walkers); n++) {
      ...
      // initialize oldPotentialShift and lastChangePotentialShift to point to
      // site 0
  ...
  if (out_file_name != NULL && out_file_name[0] != 0)
    dos_out_name = out_file_name;
  std::cout << "Wang-Landau output will be written to: " << dos_out_name
            << std::endl;
}

template <class RNG>
void WL1dEvecGenerator<RNG>::initializeEvecAndPotentialShift(
    int inst, double *evecs, double *potentialShifts) {
  for (size_t j = 0; j < n_spins; j++) potentialShifts[j] = 0.0;
  ...
  if (inst >= n_initialized_from_file) {
    if (firstFerromagnetic)
      if (inst == 0)
        ...
        for (size_t j = 0; j < 3 * n_spins; j += 3) random_evec_1(&evecs[j]);
    else
      for (size_t j = 0; j < 3 * n_spins; j += 3) random_evec_1(&evecs[j]);
  }
...
void WL1dEvecGenerator<RNG>::writeState(const char *name) {
  ...
  if (ofile) {
    ...
    if (dos.getNumberOfMoments() > 0) {
      ...
      for (int i = 0; i < dos.getN(); i++)
        ofile << dos.getNumberOfSamplesAtIdx(i)
              << ((i == dos.getN() - 1) ? "\n" : ",\n");
      ofile << "], [\n";
      for (int j = 0; j < dos.getNumberOfMoments(); j++) {
        for (int i = 0; i < dos.getN(); i++)
          ofile << dos.getMomentAtIdx(i, j)
                << ((i == dos.getN() - 1) ? "\n" : ",\n");
    ...
    for (int i = 0; i < n_walkers; i++) {
      ofile << magnetizationAtPosition[i]
            << ((i == n_walkers - 1) ? "\n" : ",\n");
    }
    ofile << "],\n";
    ofile << "\"stepsSinceLastHistogramUpdate\" : "
          << stepsSinceLastHistogramUpdate << ",\n";
    ofile << "\"numberOfUpdatesSinceLastBoost\" : "
          << numberOfUpdatesSinceLastBoost << ",\n";
    ...
    std::cerr << "# CAUTION: DoS output file could not be opened!\n";
}
...
bool WL1dEvecGenerator<RNG>::determineAcceptance(int instance, double energy,
  ...
  // +++ If a flip of spin results in energy in the same bin as previous
  // accepted
  // +++ state, it is accepted all the time. We change it by accepting if the
  // +++ new state has lower density of states according to a linear spline of
  // DOS
  ...
  // +++++++++ end of modification. Follow the new variables to see the actual
  // changes energy between xMin and xMax ?
  ...
  if (grid < 0 || grid > dos.getN() - 1) {
    ...
    if (fixEnergyWindow) {
      ...
      if (ref0[instance] < 0 || ref0[instance] > dos.getN() - 1) {
        ...
        if (grid > dos.getN() - 1 && energy - position[instance] < 0.0)
          accept_step = true;
      }
    ...
    dos.extendTo(energy - dosKernel.getWidth());
    histo.extendTo(energy - histoKernel.getWidth());
    dos.extendTo(energy + dosKernel.getWidth());
    histo.extendTo(energy + histoKernel.getWidth());
    grid = dos.idx(energy);
    // we need to adjust the grid point of the walker positions (ref0) for all
    // walkers
  ...
  if (ref0[instance] < 0 || ref0[instance] > dos.getN() - 1) {
    ...
    if (abs(ref1[instance] - ref0[instance]) < 1) {
      ...
      } else  // uphill moves
      {
        if (rnd(rng) <
            exp(to_go_or_not /
                dos.getDelta())) {  // std::cout<<" dos "<<instance<<"  weight
                                    // "<<dos.getDelta()<<std::endl;
          accept_step = true;
          ref0[instance] = ref1[instance];
          position[instance] = energy;
          magnetizationAtPosition[instance] = magnetization;
...
/*
template<class RNG>
bool WL1dEvecGenerator<RNG>::updateHistogram(int instance, double *evecs, double
energy, bool *accepted)
{
...
template <class RNG>
bool WL1dEvecGenerator<RNG>::updateHistogram(int instance, double *evecs,
                                             bool accepted) {
  ...
  if (stepsSinceLastHistogramUpdate >= flipPerUpdate) {
    ...
    if (!out[instance]) {
      ...
      if (!histogramUpdateMode)  // standard Wang-Landau
      {
        ...
        dos.addMomentsAtIdx(dos.idx(position[instance]),
                            magnetizationAtPosition[instance]);
      ...
      if (!fixEnergyWindow) {
        std::cerr << "ATTENTION: We should never reach this place in "
                     "WL1dEvecGenerator!\n";
        exit(1);
      }
    }
  ...
  if (changeMode & (8 + 16 + 32)) {
    ...
    if ((changeMode & (8 + 16)) == 8)
      n = accept;
    else if ((changeMode & (8 + 16)) == 16)
      n = reject;
    else if ((changeMode & (8 + 16)) == 8 + 16)
      n = accept + reject;

    if (changeMode & 32)
      dn = double(n) / double(n_walkers);
    else
      dn = double(n);
  ...
  if (cycleCount >= updateCycle) {
    ...
    if (!histogramUpdateMode)  // Normal Wang-Landau
    {
      ...
      // we look only at the histogram inside the energy interval that was
      // actually sampled if we use kernel updates
      ...
      if (kernelType == None) {
        ...
        histo.getMinMaxYInInterval(dos.getMinX() + dosKernel.getWidth(),
                                   dos.getMaxX() - dosKernel.getWidth(), hMin,
                                   hMax);
      }
      ...
      std::cout << "# acceptence ratio = "
                << double(accept) / double(accept + reject) << "\n";
      ...
      if (changeMode != 0 && changeMode < 8) {
        ...
        if (globalUpdate.frequency > 0 &&
            (globalUpdate.frequency * histo.getN()) < acceptSinceLastChange) {
          char fn[256];
          snprintf(fn, 255, "Dos_Global_%02d_Changes_%03d.jsn",
                   globalUpdate.changes, modificationFactorChanges);
          writeDos(fn);
          globalUpdate.changes++;
          std::cout << "# global update " << globalUpdate.changes << std::endl;
          performGlobalUpdate(dos, globalUpdate.kappa, globalUpdate.lambda,
                              globalUpdate.omega);
          histo.clear();
          acceptSinceLastChange = 0;
        } else if (((acceptSinceLastChange >= histo.getN() * updatesPerBin ||
                     !(changeMode & 1)) &&
                    (hMin >= hMinimum || !(changeMode & 2)) &&
                    (currentFlatnessCriterion > flatnessCriterion ||
                     !(changeMode & 4)))) {
          std::cout << "# level " << modificationFactorChanges
                    << " with gamma = " << gamma << " is finished.\n";
  ...
  if (accepted) {
    ...
    /* sw.writeChange(instance, numRetentions[instance], lastAccepted[instance],
    &lastAcceptedEvec[3*instance], lastAcceptedEnergy[instance]);
...
template <class RNG>
void WL1dEvecGenerator<RNG>::updateLog(int instance, double *evecs, int *occ,
                                       double energy, bool accepted,
                                       MoveChoice_t MoveChoice, bool isSpinSim,
                                       bool isOccupancySim) {
  ...
  if (isOccupancySim) {
    for (int i = 0; i < n_spins; i++) fprintf(filep, "%1d", occ[i]);
  ...
  if (lastAcceptedMove[instance] == SpinMove)
    sw.writeChange(instance, numRetentions[instance], lastAccepted[instance],
                   &lastAcceptedEvec[3 * instance],
                   lastAcceptedEnergy[instance]);
  else
    sw.writeChangeOcc(
        instance, numRetentions[instance], lastAcceptedSwap[instance].first,
        lastAcceptedSwap[instance].second, lastAcceptedOcc[instance].first,
        lastAcceptedOcc[instance].second, lastAcceptedEnergy[instance]);
...
template <class RNG>
void WL1dEvecGenerator<RNG>::generatePotentialShift(int instance,
                                                    double *potentialShifts,
                                                    bool accepted) {
  if (accepted) {
    lastChangePotentialShift[instance] = int(rnd(rng) * n_spins);
    oldPotentialShift[instance] =
        potentialShifts[lastChangePotentialShift[instance]];
    potentialShifts[lastChangePotentialShift[instance]] =
        randomPotentialShift();
  } else {
    potentialShifts[lastChangePotentialShift[instance]] =
        oldPotentialShift[instance];
    lastChangePotentialShift[instance] = int(rnd(rng) * n_spins);
...
template <class RNG>
void WL1dEvecGenerator<RNG>::generateEvec(int instance, double *evecs,
                                          bool accepted) {
  if (accepted) {
...
template <class RNG>
MoveChoice_t WL1dEvecGenerator<RNG>::selectMoveType(bool isSpinSim,
                                                    bool isOccSim) {
  ...
  if (isSpinSim) return SpinMove;
  if (isOccSim) return OccupancyMove;

  return SpinMove;
}

template <class RNG>
void WL1dEvecGenerator<RNG>::setAlloyClasses(const AlloyMixingDesc &alloyDesc,
                                             int *siteclass) {
  this->alloyDesc = alloyDesc;

  siteAlloyClass.resize(n_spins);
  for (int i = 0; i < n_spins; i++) siteAlloyClass[i] = siteclass[i];
  ...
  for (int i = 0; i < nclasses; i++) nsites[i] = 0;
  for (int i = 0; i < n_spins; i++) nsites[siteAlloyClass[i]]++;
}

template <class RNG>
void WL1dEvecGenerator<RNG>::generateOccupancies(int instance, int *occ,
                                                 bool acceptedOcc) {
  ...
  do {
    atom_i = int(rnd(rng) * n_spins);
  } while (siteAlloyClass[atom_i] != ac);
  do {
    atom_j = int(rnd(rng) * n_spins);
...
void WL1dEvecGenerator<RNG>::generateUnsampledOcc(int inst, int *occ) {
  ...
  for (int i = 0; i < atomsLeft.size(); i++) {
    ...
    for (int j = 0; j < atomsLeft[i].size(); j++) {
      ...
      if (tol < error && error < 1.0 - tol) {
        printf(
            "error: alloy concentrations are incommensurate with supercell\n");
        printf("alloy class = %d, component = %d, concentration = %f\n", i + 1,
               j + 1, alloyDesc[i][j].conc);
  ...
  for (int i = 0; i < n_spins; i++) {
    ...
    do {
      type = int(rnd(rng) * alloyDesc[ac].size());
...
void WL1dEvecGenerator<RNG>::initializeOccupancies(int inst, int *occ) {
  ...
  if (inst < n_initialized_from_file_occ) return;
