...
class InitialAtomLevel {
 public:
  ...
  int n, l;
  std::vector<Real>
      rho;  // radial charge density contribution from an electron in this level
  char type;  // 'C' for core lectron, 'S' for semi-core and 'V' for valence
  // electrons
};
...
void buildMatrix(std::vector<Real> &vr, int l, Real dr,
                 std::vector<Real> &hDiagonal,
                 std::vector<Real> &hSubdiagonal) {
  for (int i = 0; i < hDiagonal.size(); i++) {
    hDiagonal[i] =
        2.0 / (dr * dr) + vr[i] / (Real(i + 1) * dr) +
            Real(l * (l + 1)) / ((Real(i + 1) * dr) * (Real(i + 1) * dr));
  }
...
Real generateLinearRadialMesh(std::vector<Real> &r_mesh, int N, Real r0,
                              Real rN) {
...
void calculateRadialDensity(std::vector<Real> &r_mesh, Matrix<Real> &f,
                            int solution, Real energy, std::vector<Real> &rho) {
  ...
  Real normalization =
      1.0 / (4.0 * M_PI * rhoIntegrated[rhoIntegrated.size() - 1]);
...
extern "C" {
void dstevx_(const char *jobz, const char *range, int *n, double *d, double *e,
...
void findSchroedingerEigenvalues(std::vector<Real> &r_mesh,
                                 std::vector<Real> &vr, int l, int numSolutions,
...
void calculateOrbitals(
    std::vector<Real> &r_mesh, std::vector<Real> &vr, int atomicNumber,
  ...
  for (int l = 0; l <= lmax; l++) totalNumberOrbitals += nMax - l;
  ...
  for (int l = 0; l <= lmax; l++) {
    ...
    for (int i = 0; i < numStates; i++) {
      ...
      printf(" %d %2d %lg Ry", principalQuantumNumber, l,
             orbitalEnergiesAndDensities[orbitalIdx].energy);

      orbitalEnergiesAndDensities[orbitalIdx].rho.resize(r_mesh.size());
      calculateRadialDensity(r_mesh, f, i, energies[i],
                             orbitalEnergiesAndDensities[orbitalIdx].rho);
  ...
  std::sort(orbitalEnergiesAndDensities.begin(),
            orbitalEnergiesAndDensities.end(),
            [](InitialAtomLevel const &a, InitialAtomLevel const &b) {
              return a.energy < b.energy;
            });
}

void accumulateDensities(
    std::vector<InitialAtomLevel> &orbitalEnergiesAndDensities,
    int atomicNumber, std::vector<Real> &rhotot) {
  ...
  while (electronsMissing > 0) {
    if (electronsMissing >=
        2 * (2 * std::abs(orbitalEnergiesAndDensities[orbitalIdx].l) +
            1))  // a filled orbital
    {
      for (int i = 0; i < rhotot.size(); i++)
        rhotot[i] +=
            orbitalEnergiesAndDensities[orbitalIdx].rho[i] * 2 *
                (2 * std::abs(orbitalEnergiesAndDensities[orbitalIdx].l) + 1);
      electronsMissing -=
      ...
      for (int i = 0; i < rhotot.size(); i++)
        rhotot[i] +=
            orbitalEnergiesAndDensities[orbitalIdx].rho[i] * electronsMissing;

      printf("%d %2d %lg Ry: partially filled (%2d electrons)\n",
             orbitalEnergiesAndDensities[orbitalIdx].n,
             orbitalEnergiesAndDensities[orbitalIdx].l,
             orbitalEnergiesAndDensities[orbitalIdx].energy, electronsMissing);
...
void sphericalPoisson(std::vector<Real> &rho, std::vector<Real> &r_mesh,
                      std::vector<Real> &vr) {
  ...
  integrateOneDimRPower(r_mesh, rho, integral,
                        -1);  // p=-1 because rho is actually rho * r^2
  for (int i = 0; i < r_mesh.size(); i++)
    vr[i] = 2.0 * 4.0 * M_PI *
        (vr[i] + (integral[integral.size() - 1] - integral[i]) * r_mesh[i]);
}
...
Real chachiyo2016(Real rho, Real &exc) {
  ...
  Real const a =
      (std::log(2) - 1.0) /
          (M_PI * M_PI);  // converted from Hartree in Chachiyo to Rydberg
  ...
  Real rho_dex = ex / 3.0;
  //
  Real rho_dec = a * b * (rsInv + 2.0 * rsInv * rsInv) /
      (3.0 * (1.0 + b * rsInv + b * rsInv * rsInv));
...
void exchangeCorrelationPotentialLDA(std::vector<Real> &rho,
                                     std::vector<Real> &r_mesh,
                                     std::vector<Real> &vXC) {
...
void iterateOrbitals(int atomicNumber, Real atomRadius,
                     std::vector<Real> &r_mesh,
                     std::vector<InitialAtomLevel> &orbitalEnergiesAndDensities,
                     std::vector<Real> &vr) {
  ...
  for (int iter = 0; iter < maxIterations && rms > rmsTarget; iter++) {
    ...
    for (int i = 0; i < r_mesh.size(); i++)
      vrNew[i] += -2.0 * Real(atomicNumber) + vXC[i] * r_mesh[i];
...
int calculateAtomLevels(AtomData &a, std::vector<InitialAtomLevel> &atomLevels,
  ...
  iterateOrbitals(a.ztotss, a.r_mesh[a.r_mesh.size() - 1], linear_mesh,
                  atomLevels, vrLinearMesh);
  ...
  for (int i = 0; i < atomLevels.size(); i++) {
    ...
    printf("n=%2d  l=%2d  :  Energy=%12.6lf\n", atomLevels[i].n,
           atomLevels[i].l, atomLevels[i].energy);
  }
  ...
  for (int i = 0; i < a.numc; i++) {
    ...
    if (atomLevels[j].type == 'C')  // accumulate the density for core levels
    {
      for (int ir = 0; ir < a.r_mesh.size(); ir++) {
        a.corden(ir, 0) +=
            Real((3 - a.nspin) * (-a.kc(i, 0))) *
                interpolate(linear_mesh, atomLevels[j].rho, a.r_mesh[ir]);
        a.corden(ir, 1) +=
            Real((3 - a.nspin) * (-a.kc(i, 1))) *
                interpolate(linear_mesh, atomLevels[j].rho, a.r_mesh[ir]);
      }
    } else if (atomLevels[j].type ==
        'S') {  // accumulate the density for semi-core levels
      for (int ir = 0; ir < a.r_mesh.size(); ir++) {
        a.semcor(ir, 0) +=
            Real((3 - a.nspin) * (-a.kc(i, 0))) *
                interpolate(linear_mesh, atomLevels[j].rho, a.r_mesh[ir]);
  ...
  while (numValenceRemaining > 0) {
    ...
    if (2 * multiplicity <= numValenceRemaining) {
      ...
      for (int ir = 0; ir < a.r_mesh.size(); ir++) {
        rhoValence(ir, 0) +=
            0.5 * Real(numValenceRemaining * (3 - a.nspin)) *
                interpolate(linear_mesh, atomLevels[j].rho, a.r_mesh[ir]);
        rhoValence(ir, 1) +=
            0.5 * Real(numValenceRemaining * (3 - a.nspin)) *
                interpolate(linear_mesh, atomLevels[j].rho, a.r_mesh[ir]);
...
void initializeAtom(AtomData &a) {
  ...
  for (int ir = 0; ir < a.r_mesh.size(); ir++) {
    a.vr(ir, 0) = a.vr(ir, 1) =
        -2.0 * a.ztotss;  // +q*approxErfc(a.r_mesh[ir]*sigmaSqrt2Inv);
  ...
  if (coreElectrons != coreTarget) {
    printf(
        "Warning: initializeAtom can't satisfy the core electron "
        "requirement:\n  Target: %d (%lf + %lf)\n  Actual: %d\n",
        coreTarget, a.zcorss, a.zsemss, coreElectrons);
  }

  Real atomVolume =
      (4.0 / 3.0) * M_PI * a.rws * a.rws * a.rws;  // a.rmt*a.rmt*a.rmt;

  for (int ir = 0; ir < a.r_mesh.size(); ir++) {
    a.rhotot(ir, 0) = a.rhoNew(ir, 0) =
        a.corden(ir, 0) + a.semcor(ir, 0) + rhoValence(ir, 0);
    a.rhotot(ir, 1) = a.rhoNew(ir, 1) =
        a.corden(ir, 1) + a.semcor(ir, 1) + rhoValence(ir, 1);
  }
...
void initializeNewPotentials(LSMSCommunication &comm, LSMSSystemParameters &lsms,
  ...
  for (int i = 0; i < local.num_local; i++) {
    ...
    if (lsms.global.debug_atomic) {
      fmt::print("Initializing potential: {:3d}\n", i);
      fmt::print("{:>10}: {:10.6f}\n", "rmin", lsms.rmin);
      fmt::print("{:>10}: {:10.6f}\n", "rmax", lsms.rmax);
      fmt::print("{:>10}: {:10.6f}\n", "h_step", lsms.h_step);
    ...
    local.atom[i].xstart = std::log(local.atom[i].rmt) - (lsms.h_step * (local.atom[i].jmt - 1));
    local.atom[i].jws = local.atom[i].jmt;

    int last_point = std::floor(std::log(lsms.rmax) / lsms.h_step - local.atom[i].xstart / lsms.h_step) + 2;

    local.atom[i].resizePotential(last_point);
    ...
    local.atom[i].xvalws[0] = local.atom[i].xvalws[1] =
    ...
    if (lsms.global.debug_atomic) {
      ...
      fmt::print("{:>10}: {:>10d}\n", "lastpoint", local.atom[i].r_mesh.size());
      fmt::print("{:>10}: {:>10d}\n", "jmt", local.atom[i].jmt - 1);
      fmt::print("{:>10}: {:10.6f}\n", "rmt mesh", local.atom[i].r_mesh[local.atom[i].jmt - 1]);
    }
    ...
    auto i_states = n_states;

    for (i_states = 0; i_states < n_states; i_states++) {

      e_occ += occupation[i_states];

      if (e_occ > (local.atom[i].ztotss - local.atom[i].zvalss)) {
        break;
      }

    }

    local.atom[i].resizeCore(i_states);

    for (auto i_spin_pol = 0; i_spin_pol < lsms.n_spin_pola; i_spin_pol++) {
      for (auto j_states = 0; j_states < i_states; j_states++) {
        local.atom[i].ec(j_states, i_spin_pol) = ks_energies[j_states];
        local.atom[i].nc(j_states, i_spin_pol) = n[j_states];
        local.atom[i].lc(j_states, i_spin_pol) = l[j_states];
        local.atom[i].kc(j_states, i_spin_pol) = kappa[j_states];
      }
    }

    local.atom[i].numc = i_states;

    auto rSphere = local.atom[i].rmt;

    // Renormalize density inside the sphere
    for (auto i_spin_pol = 0; i_spin_pol < lsms.n_spin_pola; i_spin_pol++) {
      ...
      auto dq = local.atom[i].ztotss / lsms.n_spin_pola - norm;
      auto factor = dq * 3 / (rSphere * rSphere * rSphere);

      for (int ir = 0; ir < local.atom[i].jmt; ir++) {
        local.atom[i].rhoNew(ir, i_spin_pol) = local.atom[i].rhoNew(ir, i_spin_pol)
      ...
      if (lsms.global.debug_atomic) {
        fmt::print(" Ztot,norm,norm_scaled: {:10.6f} {:10.6f} {:10.6f}\n", full_norm, norm, norm_scaled);
      }
    ...
    if (lsms.n_spin_pola == 2) {
      ...
      Real up_factor = (local.atom[i].mag_mom * 0.5) / (local.atom[i].ztotss) + 0.5;
      Real down_factor = 1.0 - up_factor;

      for (auto ir = 0; ir < local.atom[i].r_mesh.size(); ir++) {
        local.atom[i].rhoNew(ir, 0) *= up_factor * 2.0;
      ...
      if (lsms.global.debug_atomic) {
        ...
        fmt::print("  {:10.6f} {:10.6f} {:10.6f} {:10.6f}\n",
                   norm_up,
                   norm_down,
                   norm_up - norm_down,
                   local.atom[i].mag_mom);
      }

    }

    // Copy charge density
    for (int i_spin_pol = 0; i_spin_pol < lsms.n_spin_pola; i_spin_pol++) {
      for (auto ir = 0; ir < local.atom[i].r_mesh.size(); ir++) {
        local.atom[i].rhotot(ir, i_spin_pol) = local.atom[i].rhoNew(ir, i_spin_pol);
      }
    }

    local.atom[i].alat = local.atom[i].rmt;
    local.atom[i].efermi = lsms.efermi;

  }

  lsms.chempot = lsms.efermi;
}
...
int initializeNewAlloyBank(LSMSCommunication &comm, LSMSSystemParameters &lsms,
  ...
  lsms::calculateCoreStates(comm, lsms, alloyBank);
  return 0;
}
