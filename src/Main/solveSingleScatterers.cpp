...
void solveSingleScatterers(
  ...
  for (int i = 0; i < local.num_local; i++)
    solution[i].init(lsms, local.atom[i],
                     &local.tmatStore(iie * local.blkSizeTmatStore, i));
  ...
  // if(lsms.global.iprint>=0) printf("calculate single scatterer
  // solutions.\n");
  for (int i = 0; i < local.atom.size(); i++) {
    // if(lsms.global.iprint>=1) printf("calc single scatterer
    // %d.%d\n",comm.rank,i);
    // printf("calc single scatterer atom no. = %d\n",i);
    // YingWai's check
    // printf("Inside solveSingleScatterers. Before
    // calculateSingleScatterSolution\n");
    calculateSingleScattererSolution(lsms, local.atom[i], vr[i], energy, prel,
                                     pnrel, solution[i]);
    // printf("Inside solveSingleScatterers. After
    // calculateSingleScatterSolution\n");
...
void solveSingleScatterers(
  ...
  for (int i = 0; i < local.atom.size(); i++) {
    calculateSingleScattererSolution(lsms, local.atom[i], vr[i], energy,
                                     solution[i]);
  }
  ...
  //   {
  //     printf("single site scattering t-matrix (tmat_g) for local site
  //     %d:\n",0); for(int i=0; i<solution[0].tmat_g.n_row(); i++)
  //     {
  //       printf("%2d",i);
  //       for(int j=0; j<solution[0].tmat_g.n_col(); j++)
  //         printf(" (%lf, %lf)",solution[0].tmat_g(i,j).real(),
  //         solution[0].tmat_g(i,j).imag());
  //       printf("\n");
  //     }
  //   }
  //   exit(1);
