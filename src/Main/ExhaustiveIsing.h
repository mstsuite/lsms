...
class ExhaustiveIsing1dEvecGenerator : public EvecGenerator {
 public:
  ExhaustiveIsing1dEvecGenerator(int num_spins, int num_instances,
                                 double **ev_p,
  ...
  bool updateHistogram(int instance, double *evecs) {
    std::cerr << "Need energy for ExhaustiveIsing1dEvecGenerator\n";
    exit(1);
  }
...
ExhaustiveIsing1dEvecGenerator::ExhaustiveIsing1dEvecGenerator(
    int num_spins, int num_instances, double **ev_p, const char *init_file_name,
  ...
  for (int i = 0; i < n_walkers; i++) ref0[i] = Huge;
  ...
  xMin = -Huge;
  xMax = 1.0;
  interval = 0.01;  // (xMax-xMin)/double(nX);
  ...
  if (init_file_name != NULL && init_file_name[0] != 0) {
    ...
    for (json_t *it = json_root->child; it != NULL; it = it->next) {
      ...
      else if (label == "configurationNumber")
        config_number = atoi(it->child->text);
      else if (label == "nX") {
      ...
      //*/
        ...
        if (j != nX) {
          std::cout << "ERROR #(dos) " << j << " != nX " << nX << std::endl;
          exit(1);
        }
      } else if (label == "histo") {
        json_t *a = it->child;
        ...
        if (j != nX) {
          std::cout << "ERROR #(histo) " << j << " != nX " << nX << std::endl;
          exit(1);
        }
      } else if (label == "evecs") {
        json_t *a = it->child;
...
bool ExhaustiveIsing1dEvecGenerator::updateHistogram(int instance,
                                                     double *evecs,
                                                     double energy) {
  ...
  if (grid < 0 || grid > nX - 1) {
    std::cerr << "ATTENTION: The code should never reach this point, where "
  ...
  std::cout << "Exhaustive Ising 1d EvecGenerator step " << config_number << ":"
            << numberOfUpdatesSinceLastBoost << ":"
            << stepsSinceLastHistogramUpdate << " nX=" << nX << " [" << xMin
            << ", " << xMax << "] " << std::endl;
