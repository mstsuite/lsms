...
class WL1dEvecGenerator : public EvecGenerator {
  ...
  */
  ...
  void updateLog(int instance, double *evecs, int *occ, bool accepted,
                 MoveChoice_t MoveChoice);
...
template <class RNG>
WL1dEvecGenerator<RNG>::WL1dEvecGenerator(int num_spins, int num_instances,
                                          double **ev_p,
                                          PotentialShifter &potentialShifter,
  ...
  */
  ...
  if (init_file_name != NULL && init_file_name[0] != 0) {
    ...
    // check if the input file is for start (specifies "interval") or a restart
    // (specifies "nX")
    ...
    if (data["kernelType"].type() == JSON::STRING) {
      std::string str = data["kernelType"].as_string();
      kernelType = getKernelType(str);
    }
    ...
    getValue(data, "stepsSinceLastHistogramUpdate",
             stepsSinceLastHistogramUpdate);
    getValue(data, "numberOfUpdatesSinceLastBoost",
    ...
    if (data["dos"].type() == JSON::ARRAY) {
      ...
      if (a.size() != nX) {
        std::cout << "ERROR #(dos) " << a.size() << " != nX " << dos.getN()
                  << std::endl;
        exit(1);
      }
    ...
    if (data["histo"].type() == JSON::ARRAY) {
      ...
      if (a.size() != nX) {
        std::cout << "ERROR #(histo) " << a.size() << " != nX " << histo.getN()
                  << std::endl;
        exit(1);
      }
    ...
    if (data["moments.k"].type() != JSON::NIL)
      dos.setNumberOfMoments(data["moments.k"].as_int());
    ...
    if (data["evecs"].type() != JSON::NIL) {
      ...
      for (int j = 0; j < n_initialized_from_file; j++) {
        ...
        //            oldSpin[
        //            3*n_initialized_from_file]=evecs_pointer[n_initialized_from_file][0];
  ...
  if (readPositions)
    for (int i = 0; i < n_walkers; i++) ref0[i] = dos.idx(position[i]);
...
bool WL1dEvecGenerator<RNG>::updateHistogram(int instance, double *evecs,
  ...
  if (accepted) {
    ...
    /*    sw.writeChange(instance, numRetentions[instance],
       lastAccepted[instance], &lastAcceptedEvec[3*instance],
       lastAcceptedEnergy[instance]); lastAccepted[instance] =
       lastChange[instance]; lastAcceptedEnergy[instance] = position[instance];
...
void WL1dEvecGenerator<RNG>::updateLog(int instance, double *evecs, int *occ,
                                       bool accepted, MoveChoice_t MoveChoice) {
...
MoveChoice_t WL1dEvecGenerator<RNG>::selectMoveType(bool isSpinSim,
  ...
  if (isSpinSim && isOccSim) {
    int c = int(rnd(rng) * 2);
    if (c == 0) return SpinMove else return OccupancyMove;
  }
