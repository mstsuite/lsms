...
class Graph2d {
 ...
 public:
  ...
  Graph2d(KeyType _deltaX, KeyType _deltaY)
      : deltaX(_deltaX), deltaY(_deltaY), Nx(0) {
    ;
  }
  ~Graph2d() { freeVal(); }
  void freeVal() {
  ...
  inline void extendToY(Int ix, KeyType y) {
    ...
    if (Ny[ix] < 1) {
      ...
      if (val[ix] == NULL)
        val[ix] = new std::vector<ValueType>(Ny[ix]);
      else
        val[ix]->resize(Ny[ix]);
      iy = 0;
    } else {
      ...
      if (iy < 0) {
        ...
        for (Int j = Ny[ix] - 1; j >= 0; j--) {
          (*val[ix])[j - iy] = (*val[ix])[j];
          (*val[ix])[j] = ValueType(0);
        }
        Ny[ix] = Ny[ix] - iy;
        minY[ix] += KeyType(iy) * deltaY;
        iy = 0;
      } else if (iy >= Ny[ix]) {
        Int n = iy - Ny[ix] + 1;
        val[ix]->resize(iy + 1, ValueType(0));
        Ny[ix] = iy + 1;
        maxY[ix] += KeyType(n) * deltaY;
      }
    }
  }
...
inline bool syncronizeGraphs(Graph2d<ValueType, KeyType, Int> &a,
                             Graph2d<ValueType, KeyType, Int> &b) {
...
class Kernel2d : public Graph2d<ValueType, KeyType, Int> {
 ...
 public:
  ...
  Kernel2d(KeyType _deltaX, KeyType _deltaY, KeyType _widthX, KeyType _widthY)
      : Graph2d<ValueType, KeyType, Int>(_deltaX, _deltaY),
        widthX(_widthX),
        widthY(_widthY) {
    ...
    this->extendTo(-widthX, widthY); this->extendTo(-widthX,-widthY);
    */
    this->extendToX(widthX);
    this->extendToX(-widthX);
    for (Int ix = 0; ix < this->getNx(); ix++) {
      this->extendToY(ix, widthY);
      this->extendToY(ix, -widthY);
    }
    centerX = this->idxX(KeyType(0));
    centerY = this->idxY(centerX, KeyType(0));
  }
  void setWidthAndClear(KeyType _deltaX, KeyType _deltaY, KeyType _widthX,
                        KeyType _widthY) {
...
void getKernelName(KernelType t, std::string &n) {
  switch (t) {
    ...
    case Cosine:
      ...
      break;
    default:
      std::cerr << "Unknown kernel type in Graph2d!\n";
      exit(1);
  }
}
...
template <class ValueType, class KeyType, class Int>
void initKernelFromFunction(Kernel2d<ValueType, KeyType, Int> &k,
                            ValueType (*f)(KeyType)) {
  ...
  for (Int ix = 0; ix < k.getNx(); ix++) {
    for (Int iy = 0; iy < k.getNy(ix); iy++) {
      k[ix][iy] = (*f)(
          std::sqrt(lX * k.keyFromIdxX(ix) * lX * k.keyFromIdxX(ix) +
...
template <class ValueType, class KeyType, class Int>
inline void addKernel(Graph2d<ValueType, KeyType, Int> &g,
                      Kernel2d<ValueType, KeyType, Int> &k, KeyType x,
                      KeyType y) {
  g.extendToX(x - k.getWidthX());
  g.extendToX(x + k.getWidthX());
  Int ix0 = g.idxX(x - k.getWidthX());
  for (Int ix = 0; ix < k.getNx(); ix++) {
    g.extendToY(ix + ix0, y - k.getWidthY());
    g.extendToY(ix + ix0, y + k.getWidthY());
    Int iy0 = g.idxY(ix + ix0, y - k.getWidthY());
    for (Int iy = 0; iy < k.getNy(ix); iy++) {
      // std::cout<<"ix0="<<ix0<<" iy0="<<iy0<<" ix="<<ix<<"
      // iy="<<iy<<std::endl; std::cout<<"k.Nx="<<k.getNx()<<"
      // g.Nx="<<g.getNx()<<std::endl; std::cout<<"k.Ny[ix]="<<k.getNy(ix)<<"
      // g.Ny[ix+ix0]="<<g.getNy(ix+ix0)<<std::endl;
      // std::cout<<"k[ix][iy]="<<k[ix][iy]<<std::endl;
