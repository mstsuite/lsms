...
void printLSMSSystemParameters(FILE *f, LSMSSystemParameters &lsms) {
  ...
  switch (lsms.relativity) {
    case none:
      ...
      break;
    case scalar:
      fprintf(f, "scalar relativistic\n");
      break;
    case full:
      fprintf(f, "fully relativistic\n");
      break;
    default:
      fprintf(f, "!!!!UNKNOWN!!!!\n");
  }
...
void printCrystalParameters(FILE *f, CrystalParameters &crystal) {
  fprintf(f, "Bravais lattice : %lf %lf %lf\n", crystal.bravais(0, 0),
          crystal.bravais(1, 0), crystal.bravais(2, 0));
  fprintf(f, "                : %lf %lf %lf\n", crystal.bravais(0, 1),
          crystal.bravais(1, 1), crystal.bravais(2, 1));
  fprintf(f, "                : %lf %lf %lf\n", crystal.bravais(0, 2),
          crystal.bravais(1, 2), crystal.bravais(2, 2));

  for (int i = 0; i < crystal.num_atoms; i++) {
    fprintf(f, "Atom %7d : %lf %lf %lf\n", i + 1, crystal.position(0, i),
            crystal.position(1, i), crystal.position(2, i));
    fprintf(f, "             : evec = %lf %lf %lf\n", crystal.evecs(0, i),
            crystal.evecs(1, i), crystal.evecs(2, i));
    fprintf(f, "             : type = %d\n", crystal.type[i] + 1);
  }
  for (int i = 0; i < crystal.num_types; i++) {
    fprintf(f, "Type %7d : %s [%d.%d]\n", i + 1, crystal.types[i].name,
            crystal.types[i].node, crystal.types[i].local_id);
    fprintf(f,
            "             : pot_in_idx = %d (Potential file index for input)\n",
            crystal.types[i].pot_in_idx);
    fprintf(f, "             : lmax = %d, rLIZ = %lf\n", crystal.types[i].lmax,
            crystal.types[i].rLIZ);
...
void printLocalTypeInfo(FILE *f, LocalTypeInfo &local) {
  ...
  for (int i = 0; i < local.num_local; i++) {
    ...
    fprintf(f, "              : %lf %lf %lf %lf\n", local.atom[i].vr(0, 0),
            local.atom[i].vr(0, 1), local.atom[i].vr(50, 0),
            local.atom[i].vr(50, 1));
    fprintf(f, "              : %lf %lf\n", local.atom[i].ec(0, 0),
            local.atom[i].ec(0, 1));
  ...
  for (int i = 0; i < alloy.size(); i++) {
    ...
    for (int j = 0; j < alloy[i].size(); j++) {
      ...
      fprintf(f, "    Zval : Z=%3d Zc=%3d Zs=%3d, Zv=%3d\n", alloy[i][j].Z,
              alloy[i][j].Zc, alloy[i][j].Zs, alloy[i][j].Zv);
    }
  }
}
...
void printLIZInfo(FILE *f, AtomData &atom) {
  ...
  for (int i = 0; i < atom.numLIZ; i++)
    fprintf(f, "%4d  %12.7lf %7d   %4d\n", i, atom.LIZDist[i],
            atom.LIZGlobalIdx[i], atom.LIZStoreIdx[i]);
}
