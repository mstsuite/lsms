...
struct Geom {
    ...
    Geom(Real Lx, Real Ly, Real Lz, int nx, int ny, int nz, Real Lyx=0.0, Real Lzx=0.0, Real Lzy=0.0)
        : L{Lx, Ly, Lz, Lyx,Lzx,Lzy}
        , h{ Lx/nx,  Ly/ny,  Lz/nz,
            Lyx/ny, Lzx/nz, Lzy/nz}
        , n{nx, ny, nz} {}
    ...
    inline void
    decodeBin(const unsigned int bin, int& i, int& j, int& k) const {
            i = bin % n[0];
            j = (bin/n[0])%n[1];
            k = bin/(n[0]*n[1]);
    }
    ...
    inline uint32_t
    calcBin(const int i, const int j, const int k) const {
            return (k*n[1] + j)*n[0] + i;
    }

    inline uint32_t
    calcBinF(Real &x, Real &y, Real &z) const {
