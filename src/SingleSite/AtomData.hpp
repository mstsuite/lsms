...
public:
  ...
  void get_b_basis(void) {
    ...
    norm = std::sqrt(norm2);
    b_basis[0] = b_basis[0] / norm;
    b_basis[1] = b_basis[1] / norm;
    b_basis[2] = b_basis[2] / norm;
  ...
  void newConstraint(void) {
    ...
    moment_dir[0] = evecOut[0] / moment_mag;
    moment_dir[1] = evecOut[1] / moment_mag;
...
//    b_con[2]=0.0;
  ...
  void changeNspin(int nspinNew) {
    ...
    if (nspinNew == 1) // extend from non spin polarized to spin polarized
    {
      ...
      for (int i = 0; i < exchangeCorrelationPotential.l_dim(); i++)
        exchangeCorrelationPotential(i, 0) =
            0.5 * (exchangeCorrelationPotential(i, 0) + exchangeCorrelationPotential(i, 1));
      for (int i = 0; i < exchangeCorrelationEnergy.l_dim(); i++)
	...
	kc(i,1) = kc(i,0);
      */
      ...
      ecorv[0] = ecorv[0] + ecorv[1];
      esemv[0] = esemv[0] + esemv[1];
      for (int i = 0; i < corden.l_dim(); i++)
