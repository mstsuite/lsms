...
contains
   ...
   subroutine initMixing(nt, nq, RealArrayList)
      ...
      !
      if (nt<1) then
         print *, 'initMixing', 'Number of Types is less than 1'
      endif
      !
      NumTypes = nt
   ...
   subroutine endMixing()
      ...
      if (Initialized) then
         ...
         if (MixingMethod == SimpleMixing) then
            ...
            print *, 'endMixing', 'Unknown mixing method', MixingMethod
         endif
         Initialized = .false.
      endif
      !
   ...
   subroutine mixValues(list_q)
      ...
      if (MixingMethod == SimpleMixing) then
         ...
         print *, 'mixValues', 'Unknown mixing method', MixingMethod
      endif
      !
   ...
   subroutine setSimpleMixing(idt, idq, amix)
      ...
      if (.not.Initialized) then
         print *, 'setSimpleMixing', &
            'The MixingModule needs to be initialized first'
      endif
      !
      if (idt<0 .or. idt>NumTypes) then
         print *, 'setSimpleMixing', "Invalid type index", idt
      endif
      if (idq<0 .or. idq>NumQuantities(idt)) then
         print *, 'setSimpleMixing', "Invalid quantity index", idq
      endif
      !
   ...
   subroutine setDGAMixing(idt, idq, amix)
      ...
      !
      if (idt<0 .or. idt>NumTypes) then
         print *, 'setDGAMixing', "Invalid type index", idt
      endif
      if (idq<0 .or. idq>NumQuantities(idt)) then
         print *, 'setDGAMixing', "Invalid quantity index", idq
      endif
      !
   ...
   subroutine setBroydenMixing(idt, idq, amix)
      ...
      !
      if (idt<0 .or. idt>NumTypes) then
         print *, 'setBroydenMixing', "Invalid type index", idq
      endif
      if (idq<0 .or. idq>NumQuantities(idt)) then
         print *, 'setBroydenMixing', "Invalid quantity index", idq
      endif
      !
   ...
   subroutine invert1(pa, pb, m, ptd, pad, pbd, mm)
      ...
      n = m
      if (n > mm) then
         print *, 'invert1', ' invert: matrix a too large'
      endif
      !
      do i = 1, n
         atmp = pa(i, i)
         if (abs(atmp) < ten2m8) then
            print *, "invert1", "matrix has zero diagonal(row)", i
         endif
      enddo
      !
      if (n /= 1) then
